...
const vec3 earthPos = vec3(0.0, 360010.0, 0.0); //vec3(0.0, 0.0, 6360010.0);
...
void getTransmittanceRMu(out float r, out float muS) {
    r = gl_FragCoord.z / float(TRANSMITTANCE_H);
...
void getIrradianceRMuS(out float r, out float muS) {
    r = Rg + (gl_FragCoord.z - 0.5) / (float(SKY_H) - 1.0) * (Rt - Rg);
...
/*#ifdef INSCATTER_NON_LINEAR
    if (y < float(RES_MU) / 2.0) {
        ...
        d = min(max(dhdH.x, d * dhdH.z), dhdH.z * 0.999);
...
void main() {
    ...
    vec3 extinction;
    vec3 wCamera = worldCamera.xyz;
    vec3 inscatter = skyRadiance(wCamera + earthPos, v, worldSunDir, extinction);
