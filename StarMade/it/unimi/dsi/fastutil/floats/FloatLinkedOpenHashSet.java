...
/*     */ 
/*     */ public class FloatLinkedOpenHashSet extends AbstractFloatSortedSet
...
/*     */   {
/* 182 */     this(16, f);
/* 183 */     while (i.hasNext()) add(i.nextFloat());
/*     */   }
/*     */ 
...
/*     */   {
/* 198 */     this(FloatIterators.asFloatIterator(i), f);
/*     */   }
/*     */ 
/*     */   public FloatLinkedOpenHashSet(Iterator<?> i)
/*     */   {
/* 205 */     this(FloatIterators.asFloatIterator(i));
/*     */   }
/*     */ 
...
/*     */   {
/* 250 */     int pos = HashCommon.murmurHash3(HashCommon.float2int(k)) & this.mask;
/*     */ 
...
/* 284 */         int slot = HashCommon.murmurHash3(HashCommon.float2int(this.key[pos])) & this.mask;
...
/*     */   {
/* 298 */     int pos = HashCommon.murmurHash3(HashCommon.float2int(k)) & this.mask;
/*     */ 
...
/*     */   {
/* 315 */     int pos = HashCommon.murmurHash3(HashCommon.float2int(k)) & this.mask;
/*     */ 
...
/*     */ 
/* 405 */     int pos = HashCommon.murmurHash3(HashCommon.float2int(k)) & mask;
/*     */ 
...
/*     */ 
/* 440 */     int pos = HashCommon.murmurHash3(HashCommon.float2int(k)) & mask;
/*     */ 
...
/*     */   }
/* 584 */   public FloatSortedSet tailSet(float from) { throw new UnsupportedOperationException(); } 
/* 585 */   public FloatSortedSet headSet(float to) { throw new UnsupportedOperationException(); } 
/* 586 */   public FloatSortedSet subSet(float from, float to) { throw new UnsupportedOperationException(); } 
/* 587 */   public FloatComparator comparator() { return null; }
/*     */ 
/*     */ 
...
/* 802 */     float[] newKey = new float[newN];
...
/* 808 */       float k = key[i];
/* 809 */       int pos = HashCommon.murmurHash3(HashCommon.float2int(k)) & newMask;
...
/*     */     try
/*     */     {
/* 849 */       c = (FloatLinkedOpenHashSet)super.clone();
/*     */     }
/*     */     catch (CloneNotSupportedException cantHappen) {
/* 852 */       throw new InternalError();
/*     */     }
/* 854 */     c.key = ((float[])this.key.clone());
...
/* 872 */       h += HashCommon.float2int(this.key[i]);
/* 873 */       i++;
/*     */     }
/* 875 */     return h; } 
/* 878 */   private void writeObject(ObjectOutputStream s) throws IOException { FloatIterator i = iterator();
/* 879 */     s.defaultWriteObject();
/* 880 */     for (int j = this.size; j-- != 0; s.writeFloat(i.nextFloat()));
/*     */   }
/*     */ 
...
/* 888 */     float[] key = this.key = new float[this.n];
...
/* 895 */       float k = s.readFloat();
/* 896 */       pos = HashCommon.murmurHash3(HashCommon.float2int(k)) & this.mask;
...
/* 602 */     int index = -1;
/*     */ 
/* 604 */     SetIterator() { this.next = FloatLinkedOpenHashSet.this.first;
/* 605 */       this.index = 0; }
/*     */ 
/*     */     SetIterator(float from) {
/* 608 */       if (FloatLinkedOpenHashSet.this.key[FloatLinkedOpenHashSet.this.last] == from) {
/* 609 */         this.prev = FloatLinkedOpenHashSet.this.last;
/* 610 */         this.index = FloatLinkedOpenHashSet.this.size;
/*     */       }
/*     */       else
/*     */       {
/* 614 */         int pos = HashCommon.murmurHash3(HashCommon.float2int(from)) & FloatLinkedOpenHashSet.this.mask;
/*     */ 
/* 616 */         while (FloatLinkedOpenHashSet.this.used[pos] != 0) {
/* 617 */           if (FloatLinkedOpenHashSet.this.key[pos] == from)
/*     */           {
/* 619 */             this.next = ((int)FloatLinkedOpenHashSet.this.link[pos]);
/* 620 */             this.prev = pos;
/* 621 */             return;
/*     */           }
/* 623 */           pos = pos + 1 & FloatLinkedOpenHashSet.this.mask;
/*     */         }
...
/* 633 */       this.next = ((int)FloatLinkedOpenHashSet.this.link[this.curr]);
/* 634 */       this.prev = this.curr;
/* 635 */       if (this.index >= 0) this.index += 1;
/*     */ 
/* 637 */       return FloatLinkedOpenHashSet.this.key[this.curr];
/*     */     }
/*     */     public float previousFloat() {
/* 640 */       if (!hasPrevious()) throw new NoSuchElementException();
/* 641 */       this.curr = this.prev;
/* 642 */       this.prev = ((int)(FloatLinkedOpenHashSet.this.link[this.curr] >>> 32));
/* 643 */       this.next = this.curr;
/* 644 */       if (this.index >= 0) this.index -= 1;
/* 645 */       return FloatLinkedOpenHashSet.this.key[this.curr];
/*     */     }
...
/*     */       }
/* 653 */       if (this.next == -1) {
/* 654 */         this.index = FloatLinkedOpenHashSet.this.size;
/* 655 */         return;
/*     */       }
/* 657 */       int pos = FloatLinkedOpenHashSet.this.first;
/* 658 */       this.index = 1;
/* 659 */       while (pos != this.prev) {
/* 660 */         pos = (int)FloatLinkedOpenHashSet.this.link[pos];
/* 661 */         this.index += 1;
/*     */       }
/*     */     }
/*     */ 
...
/*     */       {
/* 679 */         this.index -= 1;
/* 680 */         this.prev = ((int)(FloatLinkedOpenHashSet.this.link[this.curr] >>> 32));
/*     */       }
/*     */       else {
/* 683 */         this.next = ((int)FloatLinkedOpenHashSet.this.link[this.curr]);
/* 684 */       }FloatLinkedOpenHashSet.this.size -= 1;
/*     */ 
/* 687 */       if (this.prev == -1) FloatLinkedOpenHashSet.this.first = this.next;
/*     */       else
/* 689 */         FloatLinkedOpenHashSet.this.link[this.prev] ^= (FloatLinkedOpenHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFF) & 0xFFFFFFFF;
/* 690 */       if (this.next == -1) FloatLinkedOpenHashSet.this.last = this.prev;
/*     */       else
/* 692 */         FloatLinkedOpenHashSet.this.link[this.next] ^= (FloatLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFF) << 32) & 0x0; int pos = this.curr;
/*     */       int last;
/*     */       while (true) {
/* 696 */         pos = (last = pos) + 1 & FloatLinkedOpenHashSet.this.mask;
/* 697 */         while (FloatLinkedOpenHashSet.this.used[pos] != 0) {
/* 698 */           int slot = HashCommon.murmurHash3(HashCommon.float2int(FloatLinkedOpenHashSet.this.key[pos])) & FloatLinkedOpenHashSet.this.mask;
/* 699 */           if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) break;
/* 700 */           pos = pos + 1 & FloatLinkedOpenHashSet.this.mask;
/*     */         }
/* 702 */         if (FloatLinkedOpenHashSet.this.used[pos] == 0) break;
/* 703 */         FloatLinkedOpenHashSet.this.key[last] = FloatLinkedOpenHashSet.this.key[pos];
/* 704 */         if (this.next == pos) this.next = last;
/* 705 */         if (this.prev == pos) this.prev = last;
/* 706 */         FloatLinkedOpenHashSet.this.fixPointers(pos, last);
/*     */       }
/* 708 */       FloatLinkedOpenHashSet.this.used[last] = false;
/* 709 */       this.curr = -1;
/*     */     }
/*     */   }
/*     */ }
