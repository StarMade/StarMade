...
/*     */ 
/*     */ public class FloatOpenHashBigSet extends AbstractFloatSet
...
/* 114 */     this.key = FloatBigArrays.newBigArray(this.n);
...
/*     */   {
/* 184 */     this(16L, f);
/* 185 */     while (i.hasNext()) add(i.nextFloat());
/*     */   }
/*     */ 
...
/*     */   {
/* 207 */     this(FloatIterators.asFloatIterator(i), f);
/*     */   }
/*     */ 
/*     */   public FloatOpenHashBigSet(Iterator<?> i)
/*     */   {
/* 215 */     this(FloatIterators.asFloatIterator(i));
/*     */   }
/*     */ 
...
/* 267 */     long h = HashCommon.murmurHash3(HashCommon.float2int(k));
/*     */ 
...
/* 305 */         long slot = HashCommon.murmurHash3(HashCommon.float2int(FloatBigArrays.get(this.key, pos))) & this.mask;
...
/*     */         break;
/* 312 */       FloatBigArrays.set(this.key, last, FloatBigArrays.get(this.key, pos));
/*     */     }
/*     */ 
...
/*     */   {
/* 324 */     long h = HashCommon.murmurHash3(HashCommon.float2int(k));
/*     */ 
...
/*     */   {
/* 346 */     long h = HashCommon.murmurHash3(HashCommon.float2int(k));
/*     */ 
...
/* 548 */     float[][] key = this.key;
/* 549 */     boolean[][] newUsed = BooleanBigArrays.newBigArray(newN);
/* 550 */     float[][] newKey = FloatBigArrays.newBigArray(newN);
...
/* 559 */       float k = key[base][displ];
/* 560 */       long h = HashCommon.murmurHash3(HashCommon.float2int(k));
/*     */ 
...
/*     */     try
/*     */     {
/* 596 */       c = (FloatOpenHashBigSet)super.clone();
/*     */     }
/*     */     catch (CloneNotSupportedException cantHappen) {
/* 599 */       throw new InternalError();
/*     */     }
/* 601 */     c.key = FloatBigArrays.copy(this.key);
...
/* 621 */       h += HashCommon.float2int(key[base][displ]);
/* 622 */       base += ((displ = displ + 1 & this.segmentMask) == 0 ? 1 : 0);
/*     */     }
/* 624 */     return h; } 
/* 627 */   private void writeObject(ObjectOutputStream s) throws IOException { FloatIterator i = iterator();
/* 628 */     s.defaultWriteObject();
/* 629 */     for (long j = this.size; j-- != 0L; s.writeFloat(i.nextFloat()));
/*     */   }
/*     */ 
...
/* 636 */     float[][] key = this.key = FloatBigArrays.newBigArray(this.n);
/* 637 */     boolean[][] used = this.used = BooleanBigArrays.newBigArray(this.n);
/* 638 */     initMasks();
/*     */ 
/* 642 */     for (long i = this.size; i-- != 0L; ) {
/* 643 */       float k = s.readFloat();
/* 644 */       long h = HashCommon.murmurHash3(HashCommon.float2int(k));
...
/*     */     private SetIterator()
/*     */     {
/* 385 */       this.c = FloatOpenHashBigSet.this.size;
/*     */ 
/* 390 */       this.base = FloatOpenHashBigSet.this.key.length;
/* 391 */       this.lastBase = -1;
/* 392 */       boolean[][] used = FloatOpenHashBigSet.this.used;
/* 393 */       if (this.c != 0L)
/*     */         do if (this.displ-- == 0) {
/* 395 */             this.base -= 1;
/* 396 */             this.displ = ((int)FloatOpenHashBigSet.this.mask);
/*     */           }
...
/* 405 */       this.c -= 1L;
/*     */ 
/* 407 */       if (this.base < 0) return this.wrapped.getFloat(-(this.lastBase = --this.base) - 2);
/* 408 */       float retVal = FloatOpenHashBigSet.this.key[(this.lastBase = this.base)][(this.lastDispl = this.displ)];
/* 409 */       if (this.c != 0L) {
/* 410 */         boolean[][] used = FloatOpenHashBigSet.this.used;
/*     */         do
/* 412 */           if (this.displ-- == 0) {
/* 413 */             if (this.base-- == 0) break;
/* 414 */             this.displ = ((int)FloatOpenHashBigSet.this.mask);
/*     */           }
...
/*     */       while (true)
/*     */       {
/* 436 */         pos = (last = pos) + 1L & FloatOpenHashBigSet.this.mask;
/* 437 */         while (BooleanBigArrays.get(FloatOpenHashBigSet.this.used, pos)) {
/* 438 */           long slot = HashCommon.murmurHash3(HashCommon.float2int(FloatBigArrays.get(FloatOpenHashBigSet.this.key, pos))) & FloatOpenHashBigSet.this.mask;
/* 439 */           if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) break;
/* 440 */           pos = pos + 1L & FloatOpenHashBigSet.this.mask;
/*     */         }
/* 442 */         if (!BooleanBigArrays.get(FloatOpenHashBigSet.this.used, pos)) break;
/* 443 */         if (pos < last)
/*     */         {
/* 445 */           if (this.wrapped == null) this.wrapped = new FloatArrayList();
/* 446 */           this.wrapped.add(FloatBigArrays.get(FloatOpenHashBigSet.this.key, pos));
/*     */         }
/* 448 */         FloatBigArrays.set(FloatOpenHashBigSet.this.key, last, FloatBigArrays.get(FloatOpenHashBigSet.this.key, pos));
/*     */       }
/* 450 */       BooleanBigArrays.set(FloatOpenHashBigSet.this.used, last, false);
/* 451 */       return last;
/*     */     }
/*     */ 
/*     */     public void remove() {
...
/* 456 */       if (this.base < -1)
/*     */       {
/* 458 */         FloatOpenHashBigSet.this.remove(this.wrapped.getFloat(-this.base - 2));
/* 459 */         this.lastBase = -1;
/* 460 */         return;
/*     */       }
/* 462 */       FloatOpenHashBigSet.this.size -= 1L;
