  ...
  implements Serializable, Cloneable, Hash
{
  ...
  public CharLinkedOpenHashSet(int expected, float local_f)
  {
  ...
  public CharLinkedOpenHashSet(Collection<? extends Character> local_c, float local_f)
  {
    this(local_c.size(), local_f);
    addAll(local_c);
  }
  
  public CharLinkedOpenHashSet(Collection<? extends Character> local_c)
  {
    this(local_c, 0.75F);
  }
  
  public CharLinkedOpenHashSet(CharCollection local_c, float local_f)
  {
    this(local_c.size(), local_f);
    addAll(local_c);
  }
  
  public CharLinkedOpenHashSet(CharCollection local_c)
  {
    this(local_c, 0.75F);
  }
  
  public CharLinkedOpenHashSet(CharIterator local_i, float local_f)
  {
    this(16, local_f);
  ...
  public CharLinkedOpenHashSet(CharIterator local_i)
  {
    this(local_i, 0.75F);
  }
  
  public CharLinkedOpenHashSet(Iterator<?> local_i, float local_f)
  {
    this(CharIterators.asCharIterator(local_i), local_f);
  }
  
  public CharLinkedOpenHashSet(Iterator<?> local_i)
  {
    this(CharIterators.asCharIterator(local_i));
  }
  
  public CharLinkedOpenHashSet(char[] local_a, int offset, int length, float local_f)
  {
  ...
  public CharLinkedOpenHashSet(char[] local_a, int offset, int length)
  {
    this(local_a, offset, length, 0.75F);
  }
  
  public CharLinkedOpenHashSet(char[] local_a, float local_f)
  {
    this(local_a, 0, local_a.length, local_f);
  }
  
  public CharLinkedOpenHashSet(char[] local_a)
  {
    this(local_a, 0.75F);
  }
  ...
  {
    CharLinkedOpenHashSet local_c;
    try
    {
      local_c = (CharLinkedOpenHashSet)super.clone();
    }
    ...
    extends AbstractCharListIterator
  {
    ...
    SetIterator(char from)
    {
      ...
      {
        for (int pos = HashCommon.murmurHash3(from) & CharLinkedOpenHashSet.this.mask; CharLinkedOpenHashSet.this.used[pos] != 0; pos = pos + 1 & CharLinkedOpenHashSet.this.mask) {
          if (CharLinkedOpenHashSet.this.key[pos] == from)
          {
    ...
    public void remove()
    {
      ...
      CharLinkedOpenHashSet.this.size -= 1;
      if (this.prev == -1) {
        CharLinkedOpenHashSet.this.first = this.next;
      } else {
        CharLinkedOpenHashSet.this.link[this.prev] ^= (CharLinkedOpenHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFF) & 0xFFFFFFFF;
      }
      if (this.next == -1) {
        CharLinkedOpenHashSet.this.last = this.prev;
      } else {
        CharLinkedOpenHashSet.this.link[this.next] ^= (CharLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFF) << 32) & 0x0;
      }
      int pos = this.curr;
      int last;
      for (;;)
      {
        for (pos = (last = pos) + 1 & CharLinkedOpenHashSet.this.mask; CharLinkedOpenHashSet.this.used[pos] != 0; pos = pos + 1 & CharLinkedOpenHashSet.this.mask)
        {
          int slot = HashCommon.murmurHash3(CharLinkedOpenHashSet.this.key[pos]) & CharLinkedOpenHashSet.this.mask;
          if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) {
            break;
          }
        }
        if (CharLinkedOpenHashSet.this.used[pos] == 0) {
          break;
        }
        CharLinkedOpenHashSet.this.key[last] = CharLinkedOpenHashSet.this.key[pos];
        if (this.next == pos) {
          this.next = last;
        }
        if (this.prev == pos) {
      ...
      CharLinkedOpenHashSet.this.used[last] = false;
      this.curr = -1;
    }
  }
}
