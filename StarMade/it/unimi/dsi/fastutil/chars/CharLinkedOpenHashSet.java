...
/*     */ 
/*     */ public class CharLinkedOpenHashSet extends AbstractCharSortedSet
...
/*     */   {
/* 182 */     this(16, f);
/* 183 */     while (i.hasNext()) add(i.nextChar());
/*     */   }
/*     */ 
...
/*     */   {
/* 198 */     this(CharIterators.asCharIterator(i), f);
/*     */   }
/*     */ 
/*     */   public CharLinkedOpenHashSet(Iterator<?> i)
/*     */   {
/* 205 */     this(CharIterators.asCharIterator(i));
/*     */   }
/*     */ 
...
/*     */   }
/* 584 */   public CharSortedSet tailSet(char from) { throw new UnsupportedOperationException(); } 
/* 585 */   public CharSortedSet headSet(char to) { throw new UnsupportedOperationException(); } 
/* 586 */   public CharSortedSet subSet(char from, char to) { throw new UnsupportedOperationException(); } 
/* 587 */   public CharComparator comparator() { return null; }
/*     */ 
/*     */ 
...
/*     */     try
/*     */     {
/* 849 */       c = (CharLinkedOpenHashSet)super.clone();
/*     */     }
/*     */     catch (CloneNotSupportedException cantHappen) {
/* 852 */       throw new InternalError();
/*     */     }
/* 854 */     c.key = ((char[])this.key.clone());
...
/*     */     }
/* 875 */     return h; } 
/* 878 */   private void writeObject(ObjectOutputStream s) throws IOException { CharIterator i = iterator();
/* 879 */     s.defaultWriteObject();
/* 880 */     for (int j = this.size; j-- != 0; s.writeChar(i.nextChar()));
/*     */   }
/*     */ 
...
/* 888 */     char[] key = this.key = new char[this.n];
...
/* 602 */     int index = -1;
/*     */ 
/* 604 */     SetIterator() { this.next = CharLinkedOpenHashSet.this.first;
/* 605 */       this.index = 0; }
/*     */ 
/*     */     SetIterator(char from) {
/* 608 */       if (CharLinkedOpenHashSet.this.key[CharLinkedOpenHashSet.this.last] == from) {
/* 609 */         this.prev = CharLinkedOpenHashSet.this.last;
/* 610 */         this.index = CharLinkedOpenHashSet.this.size;
/*     */       }
/*     */       else
/*     */       {
/* 614 */         int pos = HashCommon.murmurHash3(from) & CharLinkedOpenHashSet.this.mask;
/*     */ 
/* 616 */         while (CharLinkedOpenHashSet.this.used[pos] != 0) {
/* 617 */           if (CharLinkedOpenHashSet.this.key[pos] == from)
/*     */           {
/* 619 */             this.next = ((int)CharLinkedOpenHashSet.this.link[pos]);
/* 620 */             this.prev = pos;
/* 621 */             return;
/*     */           }
/* 623 */           pos = pos + 1 & CharLinkedOpenHashSet.this.mask;
/*     */         }
...
/* 633 */       this.next = ((int)CharLinkedOpenHashSet.this.link[this.curr]);
/* 634 */       this.prev = this.curr;
/* 635 */       if (this.index >= 0) this.index += 1;
/*     */ 
/* 637 */       return CharLinkedOpenHashSet.this.key[this.curr];
/*     */     }
/*     */     public char previousChar() {
/* 640 */       if (!hasPrevious()) throw new NoSuchElementException();
/* 641 */       this.curr = this.prev;
/* 642 */       this.prev = ((int)(CharLinkedOpenHashSet.this.link[this.curr] >>> 32));
/* 643 */       this.next = this.curr;
/* 644 */       if (this.index >= 0) this.index -= 1;
/* 645 */       return CharLinkedOpenHashSet.this.key[this.curr];
/*     */     }
...
/*     */       }
/* 653 */       if (this.next == -1) {
/* 654 */         this.index = CharLinkedOpenHashSet.this.size;
/* 655 */         return;
/*     */       }
/* 657 */       int pos = CharLinkedOpenHashSet.this.first;
/* 658 */       this.index = 1;
/* 659 */       while (pos != this.prev) {
/* 660 */         pos = (int)CharLinkedOpenHashSet.this.link[pos];
/* 661 */         this.index += 1;
/*     */       }
/*     */     }
/*     */ 
...
/*     */       {
/* 679 */         this.index -= 1;
/* 680 */         this.prev = ((int)(CharLinkedOpenHashSet.this.link[this.curr] >>> 32));
/*     */       }
/*     */       else {
/* 683 */         this.next = ((int)CharLinkedOpenHashSet.this.link[this.curr]);
/* 684 */       }CharLinkedOpenHashSet.this.size -= 1;
/*     */ 
/* 687 */       if (this.prev == -1) CharLinkedOpenHashSet.this.first = this.next;
/*     */       else
/* 689 */         CharLinkedOpenHashSet.this.link[this.prev] ^= (CharLinkedOpenHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFF) & 0xFFFFFFFF;
/* 690 */       if (this.next == -1) CharLinkedOpenHashSet.this.last = this.prev;
/*     */       else
/* 692 */         CharLinkedOpenHashSet.this.link[this.next] ^= (CharLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFF) << 32) & 0x0; int pos = this.curr;
/*     */       int last;
/*     */       while (true) {
/* 696 */         pos = (last = pos) + 1 & CharLinkedOpenHashSet.this.mask;
/* 697 */         while (CharLinkedOpenHashSet.this.used[pos] != 0) {
/* 698 */           int slot = HashCommon.murmurHash3(CharLinkedOpenHashSet.this.key[pos]) & CharLinkedOpenHashSet.this.mask;
/* 699 */           if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) break;
/* 700 */           pos = pos + 1 & CharLinkedOpenHashSet.this.mask;
/*     */         }
/* 702 */         if (CharLinkedOpenHashSet.this.used[pos] == 0) break;
/* 703 */         CharLinkedOpenHashSet.this.key[last] = CharLinkedOpenHashSet.this.key[pos];
/* 704 */         if (this.next == pos) this.next = last;
/* 705 */         if (this.prev == pos) this.prev = last;
/* 706 */         CharLinkedOpenHashSet.this.fixPointers(pos, last);
/*     */       }
/* 708 */       CharLinkedOpenHashSet.this.used[last] = false;
/* 709 */       this.curr = -1;
/*     */     }
/*     */   }
/*     */ }
