...
{
  public static int downHeap(char[] heap, int size, int local_i, CharComparator local_c)
  {
    if (local_i >= size) {
      throw new IllegalArgumentException("Heap position (" + local_i + ") is larger than or equal to heap size (" + size + ")");
    }
    char local_e = heap[local_i];
    if (local_c == null)
    {
      int child;
  ...
  public static int upHeap(char[] heap, int size, int local_i, CharComparator local_c)
  {
    if (local_i >= size) {
      throw new IllegalArgumentException("Heap position (" + local_i + ") is larger than or equal to heap size (" + size + ")");
    }
  ...
  public static void makeHeap(char[] heap, int size, CharComparator local_c)
  {
    int local_i = size / 2;
    while (local_i-- != 0) {
 ...
 * Qualified Name:     it.unimi.dsi.fastutil.chars.CharHeaps
