...
/*     */ 
/*     */ public class DoubleLinkedOpenHashSet extends AbstractDoubleSortedSet
...
/* 125 */     this.key = new double[this.n];
...
/*     */   {
/* 182 */     this(16, f);
/* 183 */     while (i.hasNext()) add(i.nextDouble());
/*     */   }
/*     */ 
...
/*     */   {
/* 198 */     this(DoubleIterators.asDoubleIterator(i), f);
/*     */   }
/*     */ 
/*     */   public DoubleLinkedOpenHashSet(Iterator<?> i)
/*     */   {
/* 205 */     this(DoubleIterators.asDoubleIterator(i));
/*     */   }
/*     */ 
...
/*     */   {
/* 250 */     int pos = (int)HashCommon.murmurHash3(Double.doubleToRawLongBits(k)) & this.mask;
/*     */ 
...
/* 284 */         int slot = (int)HashCommon.murmurHash3(Double.doubleToRawLongBits(this.key[pos])) & this.mask;
...
/*     */   {
/* 298 */     int pos = (int)HashCommon.murmurHash3(Double.doubleToRawLongBits(k)) & this.mask;
/*     */ 
...
/*     */   {
/* 315 */     int pos = (int)HashCommon.murmurHash3(Double.doubleToRawLongBits(k)) & this.mask;
/*     */ 
...
/*     */ 
/* 405 */     int pos = (int)HashCommon.murmurHash3(Double.doubleToRawLongBits(k)) & mask;
/*     */ 
...
/*     */ 
/* 440 */     int pos = (int)HashCommon.murmurHash3(Double.doubleToRawLongBits(k)) & mask;
/*     */ 
...
/*     */   }
/* 584 */   public DoubleSortedSet tailSet(double from) { throw new UnsupportedOperationException(); } 
/* 585 */   public DoubleSortedSet headSet(double to) { throw new UnsupportedOperationException(); } 
/* 586 */   public DoubleSortedSet subSet(double from, double to) { throw new UnsupportedOperationException(); } 
/* 587 */   public DoubleComparator comparator() { return null; }
/*     */ 
/*     */ 
...
/* 802 */     double[] newKey = new double[newN];
...
/* 808 */       double k = key[i];
/* 809 */       int pos = (int)HashCommon.murmurHash3(Double.doubleToRawLongBits(k)) & newMask;
...
/*     */     try
/*     */     {
/* 849 */       c = (DoubleLinkedOpenHashSet)super.clone();
/*     */     }
/*     */     catch (CloneNotSupportedException cantHappen) {
/* 852 */       throw new InternalError();
/*     */     }
/* 854 */     c.key = ((double[])this.key.clone());
...
/* 872 */       h += HashCommon.double2int(this.key[i]);
/* 873 */       i++;
/*     */     }
/* 875 */     return h; } 
/* 878 */   private void writeObject(ObjectOutputStream s) throws IOException { DoubleIterator i = iterator();
/* 879 */     s.defaultWriteObject();
/* 880 */     for (int j = this.size; j-- != 0; s.writeDouble(i.nextDouble()));
/*     */   }
/*     */ 
...
/* 888 */     double[] key = this.key = new double[this.n];
...
/* 895 */       double k = s.readDouble();
/* 896 */       pos = (int)HashCommon.murmurHash3(Double.doubleToRawLongBits(k)) & this.mask;
...
/* 602 */     int index = -1;
/*     */ 
/* 604 */     SetIterator() { this.next = DoubleLinkedOpenHashSet.this.first;
/* 605 */       this.index = 0; }
/*     */ 
/*     */     SetIterator(double from) {
/* 608 */       if (DoubleLinkedOpenHashSet.this.key[DoubleLinkedOpenHashSet.this.last] == from) {
/* 609 */         this.prev = DoubleLinkedOpenHashSet.this.last;
/* 610 */         this.index = DoubleLinkedOpenHashSet.this.size;
/*     */       }
/*     */       else
/*     */       {
/* 614 */         int pos = (int)HashCommon.murmurHash3(Double.doubleToRawLongBits(from)) & DoubleLinkedOpenHashSet.this.mask;
/*     */ 
/* 616 */         while (DoubleLinkedOpenHashSet.this.used[pos] != 0) {
/* 617 */           if (DoubleLinkedOpenHashSet.this.key[pos] == from)
/*     */           {
/* 619 */             this.next = ((int)DoubleLinkedOpenHashSet.this.link[pos]);
/* 620 */             this.prev = pos;
/* 621 */             return;
/*     */           }
/* 623 */           pos = pos + 1 & DoubleLinkedOpenHashSet.this.mask;
/*     */         }
...
/* 633 */       this.next = ((int)DoubleLinkedOpenHashSet.this.link[this.curr]);
/* 634 */       this.prev = this.curr;
/* 635 */       if (this.index >= 0) this.index += 1;
/*     */ 
/* 637 */       return DoubleLinkedOpenHashSet.this.key[this.curr];
/*     */     }
/*     */     public double previousDouble() {
/* 640 */       if (!hasPrevious()) throw new NoSuchElementException();
/* 641 */       this.curr = this.prev;
/* 642 */       this.prev = ((int)(DoubleLinkedOpenHashSet.this.link[this.curr] >>> 32));
/* 643 */       this.next = this.curr;
/* 644 */       if (this.index >= 0) this.index -= 1;
/* 645 */       return DoubleLinkedOpenHashSet.this.key[this.curr];
/*     */     }
...
/*     */       }
/* 653 */       if (this.next == -1) {
/* 654 */         this.index = DoubleLinkedOpenHashSet.this.size;
/* 655 */         return;
/*     */       }
/* 657 */       int pos = DoubleLinkedOpenHashSet.this.first;
/* 658 */       this.index = 1;
/* 659 */       while (pos != this.prev) {
/* 660 */         pos = (int)DoubleLinkedOpenHashSet.this.link[pos];
/* 661 */         this.index += 1;
/*     */       }
/*     */     }
/*     */ 
...
/*     */       {
/* 679 */         this.index -= 1;
/* 680 */         this.prev = ((int)(DoubleLinkedOpenHashSet.this.link[this.curr] >>> 32));
/*     */       }
/*     */       else {
/* 683 */         this.next = ((int)DoubleLinkedOpenHashSet.this.link[this.curr]);
/* 684 */       }DoubleLinkedOpenHashSet.this.size -= 1;
/*     */ 
/* 687 */       if (this.prev == -1) DoubleLinkedOpenHashSet.this.first = this.next;
/*     */       else
/* 689 */         DoubleLinkedOpenHashSet.this.link[this.prev] ^= (DoubleLinkedOpenHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFF) & 0xFFFFFFFF;
/* 690 */       if (this.next == -1) DoubleLinkedOpenHashSet.this.last = this.prev;
/*     */       else
/* 692 */         DoubleLinkedOpenHashSet.this.link[this.next] ^= (DoubleLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFF) << 32) & 0x0; int pos = this.curr;
/*     */       int last;
/*     */       while (true) {
/* 696 */         pos = (last = pos) + 1 & DoubleLinkedOpenHashSet.this.mask;
/* 697 */         while (DoubleLinkedOpenHashSet.this.used[pos] != 0) {
/* 698 */           int slot = (int)HashCommon.murmurHash3(Double.doubleToRawLongBits(DoubleLinkedOpenHashSet.this.key[pos])) & DoubleLinkedOpenHashSet.this.mask;
/* 699 */           if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) break;
/* 700 */           pos = pos + 1 & DoubleLinkedOpenHashSet.this.mask;
/*     */         }
/* 702 */         if (DoubleLinkedOpenHashSet.this.used[pos] == 0) break;
/* 703 */         DoubleLinkedOpenHashSet.this.key[last] = DoubleLinkedOpenHashSet.this.key[pos];
/* 704 */         if (this.next == pos) this.next = last;
/* 705 */         if (this.prev == pos) this.prev = last;
/* 706 */         DoubleLinkedOpenHashSet.this.fixPointers(pos, last);
/*     */       }
/* 708 */       DoubleLinkedOpenHashSet.this.used[last] = false;
/* 709 */       this.curr = -1;
/*     */     }
/*     */   }
/*     */ }
