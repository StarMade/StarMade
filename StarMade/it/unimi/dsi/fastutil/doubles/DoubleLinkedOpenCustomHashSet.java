  ...
  implements Serializable, Cloneable, Hash
{
  ...
  public DoubleLinkedOpenCustomHashSet(int expected, float local_f, DoubleHash.Strategy strategy)
  {
  ...
  public DoubleLinkedOpenCustomHashSet(Collection<? extends Double> local_c, float local_f, DoubleHash.Strategy strategy)
  {
    this(local_c.size(), local_f, strategy);
    addAll(local_c);
  }
  
  public DoubleLinkedOpenCustomHashSet(Collection<? extends Double> local_c, DoubleHash.Strategy strategy)
  {
    this(local_c, 0.75F, strategy);
  }
  
  public DoubleLinkedOpenCustomHashSet(DoubleCollection local_c, float local_f, DoubleHash.Strategy strategy)
  {
    this(local_c.size(), local_f, strategy);
    addAll(local_c);
  }
  
  public DoubleLinkedOpenCustomHashSet(DoubleCollection local_c, DoubleHash.Strategy strategy)
  {
    this(local_c, 0.75F, strategy);
  }
  
  public DoubleLinkedOpenCustomHashSet(DoubleIterator local_i, float local_f, DoubleHash.Strategy strategy)
  {
  ...
  public DoubleLinkedOpenCustomHashSet(DoubleIterator local_i, DoubleHash.Strategy strategy)
  {
    this(local_i, 0.75F, strategy);
  }
  
  public DoubleLinkedOpenCustomHashSet(Iterator<?> local_i, float local_f, DoubleHash.Strategy strategy)
  {
    this(DoubleIterators.asDoubleIterator(local_i), local_f, strategy);
  }
  
  public DoubleLinkedOpenCustomHashSet(Iterator<?> local_i, DoubleHash.Strategy strategy)
  {
    this(DoubleIterators.asDoubleIterator(local_i), strategy);
  }
  
  public DoubleLinkedOpenCustomHashSet(double[] local_a, int offset, int length, float local_f, DoubleHash.Strategy strategy)
  {
  ...
  public DoubleLinkedOpenCustomHashSet(double[] local_a, int offset, int length, DoubleHash.Strategy strategy)
  {
    this(local_a, offset, length, 0.75F, strategy);
  }
  
  public DoubleLinkedOpenCustomHashSet(double[] local_a, float local_f, DoubleHash.Strategy strategy)
  {
    this(local_a, 0, local_a.length, local_f, strategy);
  }
  
  public DoubleLinkedOpenCustomHashSet(double[] local_a, DoubleHash.Strategy strategy)
  {
  ...
  {
    DoubleLinkedOpenCustomHashSet local_c;
    try
    {
      local_c = (DoubleLinkedOpenCustomHashSet)super.clone();
    }
    ...
    extends AbstractDoubleListIterator
  {
    ...
    SetIterator(double from)
    {
      ...
      {
        for (int pos = HashCommon.murmurHash3(DoubleLinkedOpenCustomHashSet.this.strategy.hashCode(from)) & DoubleLinkedOpenCustomHashSet.this.mask; DoubleLinkedOpenCustomHashSet.this.used[pos] != 0; pos = pos + 1 & DoubleLinkedOpenCustomHashSet.this.mask) {
    ...
    public void remove()
    {
      ...
      DoubleLinkedOpenCustomHashSet.this.size -= 1;
      if (this.prev == -1) {
        DoubleLinkedOpenCustomHashSet.this.first = this.next;
      } else {
        DoubleLinkedOpenCustomHashSet.this.link[this.prev] ^= (DoubleLinkedOpenCustomHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFF) & 0xFFFFFFFF;
      }
      if (this.next == -1) {
        DoubleLinkedOpenCustomHashSet.this.last = this.prev;
      } else {
        DoubleLinkedOpenCustomHashSet.this.link[this.next] ^= (DoubleLinkedOpenCustomHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFF) << 32) & 0x0;
      }
      int pos = this.curr;
      int last;
      for (;;)
      {
        for (pos = (last = pos) + 1 & DoubleLinkedOpenCustomHashSet.this.mask; DoubleLinkedOpenCustomHashSet.this.used[pos] != 0; pos = pos + 1 & DoubleLinkedOpenCustomHashSet.this.mask)
        {
          int slot = HashCommon.murmurHash3(DoubleLinkedOpenCustomHashSet.this.strategy.hashCode(DoubleLinkedOpenCustomHashSet.this.key[pos])) & DoubleLinkedOpenCustomHashSet.this.mask;
          if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) {
            break;
          }
        }
        if (DoubleLinkedOpenCustomHashSet.this.used[pos] == 0) {
          break;
        }
