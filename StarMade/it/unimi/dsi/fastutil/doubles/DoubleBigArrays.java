...
/*   84 */   public static final double[][] EMPTY_BIG_ARRAY = new double[0][];
/*      */ 
...
/*      */   {
/*  137 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] += 1.0D;
/*      */   }
/*      */ 
/*      */   public static void decr(double[][] array, long index)
/*      */   {
/*  145 */     array[BigArrays.segment(index)][BigArrays.displacement(index)] -= 1.0D;
/*      */   }
/*      */ 
...
/*  263 */     double[][] base = new double[baseLength][];
/*  264 */     int residual = (int)(length & 0x7FFFFFF);
/*  265 */     if (residual != 0) {
/*  266 */       for (int i = 0; i < baseLength - 1; i++) base[i] = new double[134217728];
/*  267 */       base[(baseLength - 1)] = new double[residual];
/*      */     } else {
/*  269 */       for (int i = 0; i < baseLength; i++) base[i] = new double[134217728]; 
/*      */     }
/*  270 */     return base;
/*      */   }
/*      */ 
...
/*  281 */     if (array.length <= 134217728) return new double[][] { array };
/*  282 */     double[][] bigArray = newBigArray(array.length);
...
/*  320 */       double[][] base = (double[][])Arrays.copyOf(array, baseLength);
/*  321 */       int residual = (int)(length & 0x7FFFFFF);
/*  322 */       if (residual != 0) {
/*  323 */         for (int i = valid; i < baseLength - 1; i++) base[i] = new double[134217728];
/*  324 */         base[(baseLength - 1)] = new double[residual];
/*      */       } else {
/*  326 */         for (int i = valid; i < baseLength; i++) base[i] = new double[134217728]; 
/*      */       }
...
/*  394 */     double[][] base = (double[][])Arrays.copyOf(array, baseLength);
/*  395 */     int residual = (int)(length & 0x7FFFFFF);
/*  396 */     if (residual != 0) base[(baseLength - 1)] = DoubleArrays.trim(base[(baseLength - 1)], residual);
/*  397 */     return base;
/*      */   }
/*      */ 
...
/*  430 */     double[][] a = newBigArray(length);
/*      */ 
...
/*      */   {
/*  441 */     double[][] base = (double[][])array.clone();
/*  442 */     for (int i = base.length; i-- != 0; base[i] = ((double[])array[i].clone()));
/*  443 */     return base;
/*      */   }
/*      */ 
/*      */   public static void fill(double[][] array, double value)
/*      */   {
/*  454 */     for (int i = array.length; i-- != 0; DoubleArrays.fill(array[i], value));
/*      */   }
/*      */ 
...
/*  475 */       DoubleArrays.fill(array[fromSegment], fromDispl, toDispl, value);
/*  476 */       return;
/*      */     }
/*  478 */     if (toDispl != 0) DoubleArrays.fill(array[toSegment], 0, toDispl, value); while (true) {
/*  479 */       toSegment--; if (toSegment <= fromSegment) break; DoubleArrays.fill(array[toSegment], value);
/*  480 */     }DoubleArrays.fill(array[fromSegment], fromDispl, 134217728, value);
/*      */   }
/*      */ 
/*      */   public static boolean equals(double[][] a1, double[][] a2)
/*      */   {
/*  492 */     if (length(a1) != length(a2)) return false; double[] t;
/*      */     double[] u;
/*      */     int j;
/*      */     do { int i = a1.length;
/*      */ 
...
/*      */   }
/*      */ 
/*  571 */   private static long med3(double[][] x, long a, long b, long c, DoubleComparator comp) { int ab = comp.compare(get(x, a), get(x, b));
...
/*      */   {
/*  862 */     long l = Double.doubleToRawLongBits(d);
/*  863 */     return l >= 0L ? l : l ^ 0xFFFFFFFF;
/*      */   }
/*      */ 
...
/*      */   {
/*  903 */     int maxLevel = 7;
/*  904 */     int stackSize = 1786;
/*  905 */     long[] offsetStack = new long[1786];
/*  906 */     int offsetPos = 0;
/*  907 */     long[] lengthStack = new long[1786];
/*  908 */     int lengthPos = 0;
/*  909 */     int[] levelStack = new int[1786];
/*  910 */     int levelPos = 0;
...
/*  921 */       int signMask = level % 8 == 0 ? 128 : 0;
/*  922 */       if (length < 40L) {
/*  923 */         selectionSort(a, first, first + length);
/*      */       }
/*      */       else {
/*  926 */         int shift = (7 - level % 8) * 8;
/*      */ 
/*  928 */         for (long i = length; i-- != 0L; ByteBigArrays.set(digit, i, (byte)(int)(fixDouble(get(a, first + i)) >>> shift & 0xFF ^ signMask)));
...
/*      */             }
/*      */           }
/*      */           long tmp364_363 = (p + count[i]); p = tmp364_363; pos[i] = tmp364_363;
/*      */         }
/*      */ 
...
/* 1027 */     int maxLevel = 15;
/* 1028 */     int stackSize = 3826;
/* 1029 */     long[] offsetStack = new long[3826];
/* 1030 */     int offsetPos = 0;
/* 1031 */     long[] lengthStack = new long[3826];
/* 1032 */     int lengthPos = 0;
/* 1033 */     int[] levelStack = new int[3826];
/* 1034 */     int levelPos = 0;
...
/* 1045 */       int signMask = level % 8 == 0 ? 128 : 0;
/* 1046 */       if (length < 40L) {
/* 1047 */         selectionSort(a, b, first, first + length);
/*      */       }
/*      */       else {
/* 1050 */         double[][] k = level < 8 ? a : b;
/* 1051 */         int shift = (7 - level % 8) * 8;
/*      */ 
/* 1053 */         for (long i = length; i-- != 0L; ByteBigArrays.set(digit, i, (byte)(int)(fixDouble(get(k, first + i)) >>> shift & 0xFF ^ signMask)));
...
/* 1075 */           double t = get(a, i + first);
/* 1076 */           double u = get(b, i + first);
...
/*  554 */       return DoubleBigArrays.equals(a, b);
/*      */     }
/*      */   }
/*      */ }
