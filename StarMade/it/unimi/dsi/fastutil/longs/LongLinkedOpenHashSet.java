...
/*     */ 
/*     */ public class LongLinkedOpenHashSet extends AbstractLongSortedSet
...
/*     */   {
/* 182 */     this(16, f);
/* 183 */     while (i.hasNext()) add(i.nextLong());
/*     */   }
/*     */ 
...
/*     */   {
/* 198 */     this(LongIterators.asLongIterator(i), f);
/*     */   }
/*     */ 
/*     */   public LongLinkedOpenHashSet(Iterator<?> i)
/*     */   {
/* 205 */     this(LongIterators.asLongIterator(i));
/*     */   }
/*     */ 
...
/*     */   {
/* 250 */     int pos = (int)HashCommon.murmurHash3(k) & this.mask;
/*     */ 
...
/* 284 */         int slot = (int)HashCommon.murmurHash3(this.key[pos]) & this.mask;
...
/*     */   {
/* 298 */     int pos = (int)HashCommon.murmurHash3(k) & this.mask;
/*     */ 
...
/*     */   {
/* 315 */     int pos = (int)HashCommon.murmurHash3(k) & this.mask;
/*     */ 
...
/*     */ 
/* 405 */     int pos = (int)HashCommon.murmurHash3(k) & mask;
/*     */ 
...
/*     */ 
/* 440 */     int pos = (int)HashCommon.murmurHash3(k) & mask;
/*     */ 
...
/*     */   }
/* 584 */   public LongSortedSet tailSet(long from) { throw new UnsupportedOperationException(); } 
/* 585 */   public LongSortedSet headSet(long to) { throw new UnsupportedOperationException(); } 
/* 586 */   public LongSortedSet subSet(long from, long to) { throw new UnsupportedOperationException(); } 
/* 587 */   public LongComparator comparator() { return null; }
/*     */ 
/*     */ 
...
/* 808 */       long k = key[i];
/* 809 */       int pos = (int)HashCommon.murmurHash3(k) & newMask;
...
/*     */     try
/*     */     {
/* 849 */       c = (LongLinkedOpenHashSet)super.clone();
/*     */     }
/*     */     catch (CloneNotSupportedException cantHappen) {
/* 852 */       throw new InternalError();
/*     */     }
/* 854 */     c.key = ((long[])this.key.clone());
...
/* 872 */       h += HashCommon.long2int(this.key[i]);
/* 873 */       i++;
/*     */     }
/* 875 */     return h; } 
/* 878 */   private void writeObject(ObjectOutputStream s) throws IOException { LongIterator i = iterator();
/* 879 */     s.defaultWriteObject();
/* 880 */     for (int j = this.size; j-- != 0; s.writeLong(i.nextLong()));
/*     */   }
/*     */ 
...
/* 888 */     long[] key = this.key = new long[this.n];
...
/* 895 */       long k = s.readLong();
/* 896 */       pos = (int)HashCommon.murmurHash3(k) & this.mask;
...
/* 602 */     int index = -1;
/*     */ 
/* 604 */     SetIterator() { this.next = LongLinkedOpenHashSet.this.first;
/* 605 */       this.index = 0; }
/*     */ 
/*     */     SetIterator(long from) {
/* 608 */       if (LongLinkedOpenHashSet.this.key[LongLinkedOpenHashSet.this.last] == from) {
/* 609 */         this.prev = LongLinkedOpenHashSet.this.last;
/* 610 */         this.index = LongLinkedOpenHashSet.this.size;
/*     */       }
/*     */       else
/*     */       {
/* 614 */         int pos = (int)HashCommon.murmurHash3(from) & LongLinkedOpenHashSet.this.mask;
/*     */ 
/* 616 */         while (LongLinkedOpenHashSet.this.used[pos] != 0) {
/* 617 */           if (LongLinkedOpenHashSet.this.key[pos] == from)
/*     */           {
/* 619 */             this.next = ((int)LongLinkedOpenHashSet.this.link[pos]);
/* 620 */             this.prev = pos;
/* 621 */             return;
/*     */           }
/* 623 */           pos = pos + 1 & LongLinkedOpenHashSet.this.mask;
/*     */         }
...
/* 633 */       this.next = ((int)LongLinkedOpenHashSet.this.link[this.curr]);
/* 634 */       this.prev = this.curr;
/* 635 */       if (this.index >= 0) this.index += 1;
/*     */ 
/* 637 */       return LongLinkedOpenHashSet.this.key[this.curr];
/*     */     }
/*     */     public long previousLong() {
/* 640 */       if (!hasPrevious()) throw new NoSuchElementException();
/* 641 */       this.curr = this.prev;
/* 642 */       this.prev = ((int)(LongLinkedOpenHashSet.this.link[this.curr] >>> 32));
/* 643 */       this.next = this.curr;
/* 644 */       if (this.index >= 0) this.index -= 1;
/* 645 */       return LongLinkedOpenHashSet.this.key[this.curr];
/*     */     }
...
/*     */       }
/* 653 */       if (this.next == -1) {
/* 654 */         this.index = LongLinkedOpenHashSet.this.size;
/* 655 */         return;
/*     */       }
/* 657 */       int pos = LongLinkedOpenHashSet.this.first;
/* 658 */       this.index = 1;
/* 659 */       while (pos != this.prev) {
/* 660 */         pos = (int)LongLinkedOpenHashSet.this.link[pos];
/* 661 */         this.index += 1;
/*     */       }
/*     */     }
/*     */ 
...
/*     */       {
/* 679 */         this.index -= 1;
/* 680 */         this.prev = ((int)(LongLinkedOpenHashSet.this.link[this.curr] >>> 32));
/*     */       }
/*     */       else {
/* 683 */         this.next = ((int)LongLinkedOpenHashSet.this.link[this.curr]);
/* 684 */       }LongLinkedOpenHashSet.this.size -= 1;
/*     */ 
/* 687 */       if (this.prev == -1) LongLinkedOpenHashSet.this.first = this.next;
/*     */       else
/* 689 */         LongLinkedOpenHashSet.this.link[this.prev] ^= (LongLinkedOpenHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFF) & 0xFFFFFFFF;
/* 690 */       if (this.next == -1) LongLinkedOpenHashSet.this.last = this.prev;
/*     */       else
/* 692 */         LongLinkedOpenHashSet.this.link[this.next] ^= (LongLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFF) << 32) & 0x0; int pos = this.curr;
/*     */       int last;
/*     */       while (true) {
/* 696 */         pos = (last = pos) + 1 & LongLinkedOpenHashSet.this.mask;
/* 697 */         while (LongLinkedOpenHashSet.this.used[pos] != 0) {
/* 698 */           int slot = (int)HashCommon.murmurHash3(LongLinkedOpenHashSet.this.key[pos]) & LongLinkedOpenHashSet.this.mask;
/* 699 */           if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) break;
/* 700 */           pos = pos + 1 & LongLinkedOpenHashSet.this.mask;
/*     */         }
/* 702 */         if (LongLinkedOpenHashSet.this.used[pos] == 0) break;
/* 703 */         LongLinkedOpenHashSet.this.key[last] = LongLinkedOpenHashSet.this.key[pos];
/* 704 */         if (this.next == pos) this.next = last;
/* 705 */         if (this.prev == pos) this.prev = last;
/* 706 */         LongLinkedOpenHashSet.this.fixPointers(pos, last);
/*     */       }
/* 708 */       LongLinkedOpenHashSet.this.used[last] = false;
/* 709 */       this.curr = -1;
/*     */     }
/*     */   }
/*     */ }
