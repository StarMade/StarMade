  ...
  implements Serializable, Cloneable, Hash
{
  ...
  public LongLinkedOpenHashSet(int expected, float local_f)
  {
  ...
  public LongLinkedOpenHashSet(Collection<? extends Long> local_c, float local_f)
  {
    this(local_c.size(), local_f);
    addAll(local_c);
  }
  
  public LongLinkedOpenHashSet(Collection<? extends Long> local_c)
  {
    this(local_c, 0.75F);
  }
  
  public LongLinkedOpenHashSet(LongCollection local_c, float local_f)
  {
    this(local_c.size(), local_f);
    addAll(local_c);
  }
  
  public LongLinkedOpenHashSet(LongCollection local_c)
  {
    this(local_c, 0.75F);
  }
  
  public LongLinkedOpenHashSet(LongIterator local_i, float local_f)
  {
    this(16, local_f);
  ...
  public LongLinkedOpenHashSet(LongIterator local_i)
  {
    this(local_i, 0.75F);
  }
  
  public LongLinkedOpenHashSet(Iterator<?> local_i, float local_f)
  {
    this(LongIterators.asLongIterator(local_i), local_f);
  }
  
  public LongLinkedOpenHashSet(Iterator<?> local_i)
  {
    this(LongIterators.asLongIterator(local_i));
  }
  
  public LongLinkedOpenHashSet(long[] local_a, int offset, int length, float local_f)
  {
  ...
  public LongLinkedOpenHashSet(long[] local_a, int offset, int length)
  {
    this(local_a, offset, length, 0.75F);
  }
  
  public LongLinkedOpenHashSet(long[] local_a, float local_f)
  {
    this(local_a, 0, local_a.length, local_f);
  }
  
  public LongLinkedOpenHashSet(long[] local_a)
  {
    this(local_a, 0.75F);
  }
  ...
  {
    LongLinkedOpenHashSet local_c;
    try
    {
      local_c = (LongLinkedOpenHashSet)super.clone();
    }
    ...
    extends AbstractLongListIterator
  {
    ...
    SetIterator(long from)
    {
      ...
      {
        for (int pos = (int)HashCommon.murmurHash3(from) & LongLinkedOpenHashSet.this.mask; LongLinkedOpenHashSet.this.used[pos] != 0; pos = pos + 1 & LongLinkedOpenHashSet.this.mask) {
          if (LongLinkedOpenHashSet.this.key[pos] == from)
          {
    ...
    public void remove()
    {
      ...
      LongLinkedOpenHashSet.this.size -= 1;
      if (this.prev == -1) {
        LongLinkedOpenHashSet.this.first = this.next;
      } else {
        LongLinkedOpenHashSet.this.link[this.prev] ^= (LongLinkedOpenHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFF) & 0xFFFFFFFF;
      }
      if (this.next == -1) {
        LongLinkedOpenHashSet.this.last = this.prev;
      } else {
        LongLinkedOpenHashSet.this.link[this.next] ^= (LongLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFF) << 32) & 0x0;
      }
      int pos = this.curr;
      int last;
      for (;;)
      {
        for (pos = (last = pos) + 1 & LongLinkedOpenHashSet.this.mask; LongLinkedOpenHashSet.this.used[pos] != 0; pos = pos + 1 & LongLinkedOpenHashSet.this.mask)
        {
          int slot = (int)HashCommon.murmurHash3(LongLinkedOpenHashSet.this.key[pos]) & LongLinkedOpenHashSet.this.mask;
          if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) {
            break;
          }
        }
        if (LongLinkedOpenHashSet.this.used[pos] == 0) {
          break;
        }
        LongLinkedOpenHashSet.this.key[last] = LongLinkedOpenHashSet.this.key[pos];
        if (this.next == pos) {
          this.next = last;
        }
        if (this.prev == pos) {
      ...
      LongLinkedOpenHashSet.this.used[last] = false;
      this.curr = -1;
    }
  }
}
