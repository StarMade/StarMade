...
/* 194 */     while (i.hasNext()) add(i.nextByte());
/*     */   }
/*     */ 
...
/*     */   {
/* 211 */     this(ByteIterators.asByteIterator(i), f, strategy);
/*     */   }
/*     */ 
/*     */   public ByteLinkedOpenCustomHashSet(Iterator<?> i, ByteHash.Strategy strategy)
/*     */   {
/* 219 */     this(ByteIterators.asByteIterator(i), strategy);
/*     */   }
/*     */ 
...
/* 231 */     ByteArrays.ensureOffsetLength(a, offset, length);
...
/*     */   }
/* 609 */   public ByteSortedSet tailSet(byte from) { throw new UnsupportedOperationException(); } 
/* 610 */   public ByteSortedSet headSet(byte to) { throw new UnsupportedOperationException(); } 
/* 611 */   public ByteSortedSet subSet(byte from, byte to) { throw new UnsupportedOperationException(); } 
/* 612 */   public ByteComparator comparator() { return null; }
/*     */ 
/*     */ 
...
/*     */ 
/* 825 */     byte[] key = this.key;
/* 826 */     int newMask = newN - 1;
/* 827 */     byte[] newKey = new byte[newN];
...
/*     */     try
/*     */     {
/* 874 */       c = (ByteLinkedOpenCustomHashSet)super.clone();
/*     */     }
/*     */     catch (CloneNotSupportedException cantHappen) {
/* 877 */       throw new InternalError();
/*     */     }
/* 879 */     c.key = ((byte[])this.key.clone());
...
/*     */     }
/* 901 */     return h; } 
/* 904 */   private void writeObject(ObjectOutputStream s) throws IOException { ByteIterator i = iterator();
/* 905 */     s.defaultWriteObject();
/* 906 */     for (int j = this.size; j-- != 0; s.writeByte(i.nextByte()));
/*     */   }
/*     */ 
...
/* 914 */     byte[] key = this.key = new byte[this.n];
...
/* 627 */     int index = -1;
/*     */ 
/* 629 */     SetIterator() { this.next = ByteLinkedOpenCustomHashSet.this.first;
/* 630 */       this.index = 0; }
/*     */ 
/*     */     SetIterator(byte from) {
/* 633 */       if (ByteLinkedOpenCustomHashSet.this.strategy.equals(ByteLinkedOpenCustomHashSet.this.key[ByteLinkedOpenCustomHashSet.this.last], from)) {
/* 634 */         this.prev = ByteLinkedOpenCustomHashSet.this.last;
/* 635 */         this.index = ByteLinkedOpenCustomHashSet.this.size;
/*     */       }
/*     */       else
/*     */       {
/* 639 */         int pos = HashCommon.murmurHash3(ByteLinkedOpenCustomHashSet.this.strategy.hashCode(from)) & ByteLinkedOpenCustomHashSet.this.mask;
/*     */ 
/* 641 */         while (ByteLinkedOpenCustomHashSet.this.used[pos] != 0) {
/* 642 */           if (ByteLinkedOpenCustomHashSet.this.strategy.equals(ByteLinkedOpenCustomHashSet.this.key[pos], from))
/*     */           {
/* 644 */             this.next = ((int)ByteLinkedOpenCustomHashSet.this.link[pos]);
/* 645 */             this.prev = pos;
/* 646 */             return;
/*     */           }
/* 648 */           pos = pos + 1 & ByteLinkedOpenCustomHashSet.this.mask;
/*     */         }
...
/* 658 */       this.next = ((int)ByteLinkedOpenCustomHashSet.this.link[this.curr]);
/* 659 */       this.prev = this.curr;
/* 660 */       if (this.index >= 0) this.index += 1;
/*     */ 
/* 662 */       return ByteLinkedOpenCustomHashSet.this.key[this.curr];
/*     */     }
/*     */     public byte previousByte() {
/* 665 */       if (!hasPrevious()) throw new NoSuchElementException();
/* 666 */       this.curr = this.prev;
/* 667 */       this.prev = ((int)(ByteLinkedOpenCustomHashSet.this.link[this.curr] >>> 32));
/* 668 */       this.next = this.curr;
/* 669 */       if (this.index >= 0) this.index -= 1;
/* 670 */       return ByteLinkedOpenCustomHashSet.this.key[this.curr];
/*     */     }
...
/*     */       }
/* 678 */       if (this.next == -1) {
/* 679 */         this.index = ByteLinkedOpenCustomHashSet.this.size;
/* 680 */         return;
/*     */       }
/* 682 */       int pos = ByteLinkedOpenCustomHashSet.this.first;
/* 683 */       this.index = 1;
/* 684 */       while (pos != this.prev) {
/* 685 */         pos = (int)ByteLinkedOpenCustomHashSet.this.link[pos];
/* 686 */         this.index += 1;
/*     */       }
/*     */     }
/*     */ 
...
/*     */       {
/* 704 */         this.index -= 1;
/* 705 */         this.prev = ((int)(ByteLinkedOpenCustomHashSet.this.link[this.curr] >>> 32));
/*     */       }
/*     */       else {
/* 708 */         this.next = ((int)ByteLinkedOpenCustomHashSet.this.link[this.curr]);
/* 709 */       }ByteLinkedOpenCustomHashSet.this.size -= 1;
/*     */ 
/* 712 */       if (this.prev == -1) ByteLinkedOpenCustomHashSet.this.first = this.next;
/*     */       else
/* 714 */         ByteLinkedOpenCustomHashSet.this.link[this.prev] ^= (ByteLinkedOpenCustomHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFF) & 0xFFFFFFFF;
/* 715 */       if (this.next == -1) ByteLinkedOpenCustomHashSet.this.last = this.prev;
/*     */       else
/* 717 */         ByteLinkedOpenCustomHashSet.this.link[this.next] ^= (ByteLinkedOpenCustomHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFF) << 32) & 0x0; int pos = this.curr;
/*     */       int last;
/*     */       while (true) {
/* 721 */         pos = (last = pos) + 1 & ByteLinkedOpenCustomHashSet.this.mask;
/* 722 */         while (ByteLinkedOpenCustomHashSet.this.used[pos] != 0) {
/* 723 */           int slot = HashCommon.murmurHash3(ByteLinkedOpenCustomHashSet.this.strategy.hashCode(ByteLinkedOpenCustomHashSet.this.key[pos])) & ByteLinkedOpenCustomHashSet.this.mask;
/* 724 */           if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) break;
/* 725 */           pos = pos + 1 & ByteLinkedOpenCustomHashSet.this.mask;
/*     */         }
/* 727 */         if (ByteLinkedOpenCustomHashSet.this.used[pos] == 0) break;
/* 728 */         ByteLinkedOpenCustomHashSet.this.key[last] = ByteLinkedOpenCustomHashSet.this.key[pos];
/* 729 */         if (this.next == pos) this.next = last;
/* 730 */         if (this.prev == pos) this.prev = last;
/* 731 */         ByteLinkedOpenCustomHashSet.this.fixPointers(pos, last);
/*     */       }
/* 733 */       ByteLinkedOpenCustomHashSet.this.used[last] = false;
/* 734 */       this.curr = -1;
/*     */     }
/*     */   }
/*     */ }
