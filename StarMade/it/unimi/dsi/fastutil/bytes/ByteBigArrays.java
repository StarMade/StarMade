...
/*   84 */   public static final byte[][] EMPTY_BIG_ARRAY = new byte[0][];
/*      */ 
...
/*      */   {
/*  109 */     byte t = array[BigArrays.segment(first)][BigArrays.displacement(first)];
...
/*      */     byte[] tmp10_5 = array[BigArrays.segment(index)]; tmp10_5[tmp10_7] = ((byte)(tmp10_5[tmp10_7] + tmp10_7));
/*      */   }
/*      */ 
...
/*      */   {
/*      */     int tmp10_7 = BigArrays.displacement(index);
/*      */     byte[] tmp10_5 = array[BigArrays.segment(index)]; tmp10_5[tmp10_7] = ((byte)(tmp10_5[tmp10_7] + 1));
/*      */   }
/*      */ 
...
/*  263 */     byte[][] base = new byte[baseLength][];
/*  264 */     int residual = (int)(length & 0x7FFFFFF);
/*  265 */     if (residual != 0) {
/*  266 */       for (int i = 0; i < baseLength - 1; i++) base[i] = new byte[134217728];
/*  267 */       base[(baseLength - 1)] = new byte[residual];
/*      */     } else {
/*  269 */       for (int i = 0; i < baseLength; i++) base[i] = new byte[134217728]; 
/*      */     }
/*  270 */     return base;
/*      */   }
/*      */ 
...
/*  281 */     if (array.length <= 134217728) return new byte[][] { array };
/*  282 */     byte[][] bigArray = newBigArray(array.length);
...
/*  320 */       byte[][] base = (byte[][])Arrays.copyOf(array, baseLength);
/*  321 */       int residual = (int)(length & 0x7FFFFFF);
/*  322 */       if (residual != 0) {
/*  323 */         for (int i = valid; i < baseLength - 1; i++) base[i] = new byte[134217728];
/*  324 */         base[(baseLength - 1)] = new byte[residual];
/*      */       } else {
/*  326 */         for (int i = valid; i < baseLength; i++) base[i] = new byte[134217728]; 
/*      */       }
...
/*  394 */     byte[][] base = (byte[][])Arrays.copyOf(array, baseLength);
/*  395 */     int residual = (int)(length & 0x7FFFFFF);
/*  396 */     if (residual != 0) base[(baseLength - 1)] = ByteArrays.trim(base[(baseLength - 1)], residual);
/*  397 */     return base;
/*      */   }
/*      */ 
...
/*      */   {
/*  441 */     byte[][] base = (byte[][])array.clone();
/*  442 */     for (int i = base.length; i-- != 0; base[i] = ((byte[])array[i].clone()));
/*  443 */     return base;
/*      */   }
/*      */ 
/*      */   public static void fill(byte[][] array, byte value)
/*      */   {
/*  454 */     for (int i = array.length; i-- != 0; ByteArrays.fill(array[i], value));
/*      */   }
/*      */ 
...
/*  475 */       ByteArrays.fill(array[fromSegment], fromDispl, toDispl, value);
/*  476 */       return;
/*      */     }
/*  478 */     if (toDispl != 0) ByteArrays.fill(array[toSegment], 0, toDispl, value); while (true) {
/*  479 */       toSegment--; if (toSegment <= fromSegment) break; ByteArrays.fill(array[toSegment], value);
/*  480 */     }ByteArrays.fill(array[fromSegment], fromDispl, 134217728, value);
/*      */   }
/*      */ 
/*      */   public static boolean equals(byte[][] a1, byte[][] a2)
/*      */   {
/*  492 */     if (length(a1) != length(a2)) return false; byte[] t;
/*      */     byte[] u;
/*      */     int j;
/*      */     do { int i = a1.length;
/*      */ 
...
/*      */   }
/*      */ 
/*  571 */   private static long med3(byte[][] x, long a, long b, long c, ByteComparator comp) { int ab = comp.compare(get(x, a), get(x, b));
...
/*  899 */     int maxLevel = 0;
/*  900 */     int stackSize = 1;
/*  901 */     long[] offsetStack = new long[1];
/*  902 */     int offsetPos = 0;
/*  903 */     long[] lengthStack = new long[1];
/*  904 */     int lengthPos = 0;
/*  905 */     int[] levelStack = new int[1];
/*  906 */     int levelPos = 0;
...
/*  912 */     byte[][] digit = newBigArray(to - from);
...
/*  917 */       int signMask = level % 1 == 0 ? 128 : 0;
/*  918 */       if (length < 40L) {
/*  919 */         selectionSort(a, first, first + length);
/*      */       }
/*      */       else {
/*  922 */         int shift = (0 - level % 1) * 8;
/*      */ 
/*  924 */         for (long i = length; i-- != 0L; set(digit, i, (byte)(get(a, first + i) >>> shift & 0xFF ^ signMask)));
/*  925 */         for (long i = length; i-- != 0L; count[(get(digit, i) & 0xFF)] += 1L);
/*  927 */         int lastUsed = -1;
/*  928 */         long p = 0L;
/*  929 */         for (int i = 0; i < 256; i++) {
/*  930 */           if (count[i] != 0L) {
/*  931 */             lastUsed = i;
/*  932 */             if ((level < 0) && (count[i] > 1L))
/*      */             {
...
/*      */             }
/*      */           }
/*      */           long tmp345_344 = (p + count[i]); p = tmp345_344; pos[i] = tmp345_344;
/*      */         }
/*      */ 
...
/*  947 */           byte t = get(a, i + first);
/*  948 */           c = get(digit, i) & 0xFF;
/*      */           while (true)
/*      */           {
/*      */             long d;
...
/*  951 */             int zz = c;
/*  952 */             t = get(a, d + first);
/*  953 */             c = get(digit, d) & 0xFF;
/*  954 */             set(a, d + first, z);
/*  955 */             set(digit, d, (byte)zz);
/*      */           }
/*  957 */           set(a, i + first, t);
/*      */ 
/*  946 */           i += count[c];
...
/* 1023 */     int maxLevel = 1;
/* 1024 */     int stackSize = 256;
/* 1025 */     long[] offsetStack = new long[256];
/* 1026 */     int offsetPos = 0;
/* 1027 */     long[] lengthStack = new long[256];
/* 1028 */     int lengthPos = 0;
/* 1029 */     int[] levelStack = new int[256];
/* 1030 */     int levelPos = 0;
...
/* 1036 */     byte[][] digit = newBigArray(to - from);
...
/* 1041 */       int signMask = level % 1 == 0 ? 128 : 0;
/* 1042 */       if (length < 40L) {
/* 1043 */         selectionSort(a, b, first, first + length);
/*      */       }
/*      */       else {
/* 1046 */         byte[][] k = level < 1 ? a : b;
/* 1047 */         int shift = (0 - level % 1) * 8;
/*      */ 
/* 1049 */         for (long i = length; i-- != 0L; set(digit, i, (byte)(get(k, first + i) >>> shift & 0xFF ^ signMask)));
/* 1050 */         for (long i = length; i-- != 0L; count[(get(digit, i) & 0xFF)] += 1L);
/* 1052 */         int lastUsed = -1;
/* 1053 */         long p = 0L;
/* 1054 */         for (int i = 0; i < 256; i++) {
/* 1055 */           if (count[i] != 0L) {
/* 1056 */             lastUsed = i;
/* 1057 */             if ((level < 1) && (count[i] > 1L)) {
...
/* 1071 */           byte t = get(a, i + first);
/* 1072 */           byte u = get(b, i + first);
/* 1073 */           c = get(digit, i) & 0xFF;
/*      */           while (true)
/*      */           {
/*      */             long d;
...
/* 1082 */             c = get(digit, d) & 0xFF;
/* 1083 */             set(digit, d, (byte)zz);
/*      */           }
...
/*  554 */       return ByteBigArrays.equals(a, b);
/*      */     }
/*      */   }
/*      */ }
