  ...
  implements Serializable, Cloneable, Hash
{
  ...
  public ByteLinkedOpenHashSet(int expected, float local_f)
  {
  ...
  public ByteLinkedOpenHashSet(Collection<? extends Byte> local_c, float local_f)
  {
    this(local_c.size(), local_f);
    addAll(local_c);
  }
  
  public ByteLinkedOpenHashSet(Collection<? extends Byte> local_c)
  {
    this(local_c, 0.75F);
  }
  
  public ByteLinkedOpenHashSet(ByteCollection local_c, float local_f)
  {
    this(local_c.size(), local_f);
    addAll(local_c);
  }
  
  public ByteLinkedOpenHashSet(ByteCollection local_c)
  {
    this(local_c, 0.75F);
  }
  
  public ByteLinkedOpenHashSet(ByteIterator local_i, float local_f)
  {
    this(16, local_f);
  ...
  public ByteLinkedOpenHashSet(ByteIterator local_i)
  {
    this(local_i, 0.75F);
  }
  
  public ByteLinkedOpenHashSet(Iterator<?> local_i, float local_f)
  {
    this(ByteIterators.asByteIterator(local_i), local_f);
  }
  
  public ByteLinkedOpenHashSet(Iterator<?> local_i)
  {
    this(ByteIterators.asByteIterator(local_i));
  }
  
  public ByteLinkedOpenHashSet(byte[] local_a, int offset, int length, float local_f)
  {
  ...
  public ByteLinkedOpenHashSet(byte[] local_a, int offset, int length)
  {
    this(local_a, offset, length, 0.75F);
  }
  
  public ByteLinkedOpenHashSet(byte[] local_a, float local_f)
  {
    this(local_a, 0, local_a.length, local_f);
  }
  
  public ByteLinkedOpenHashSet(byte[] local_a)
  {
    this(local_a, 0.75F);
  }
  ...
  {
    ByteLinkedOpenHashSet local_c;
    try
    {
      local_c = (ByteLinkedOpenHashSet)super.clone();
    }
    ...
    extends AbstractByteListIterator
  {
    ...
    SetIterator(byte from)
    {
      ...
      {
        for (int pos = HashCommon.murmurHash3(from) & ByteLinkedOpenHashSet.this.mask; ByteLinkedOpenHashSet.this.used[pos] != 0; pos = pos + 1 & ByteLinkedOpenHashSet.this.mask) {
          if (ByteLinkedOpenHashSet.this.key[pos] == from)
          {
    ...
    public void remove()
    {
      ...
      ByteLinkedOpenHashSet.this.size -= 1;
      if (this.prev == -1) {
        ByteLinkedOpenHashSet.this.first = this.next;
      } else {
        ByteLinkedOpenHashSet.this.link[this.prev] ^= (ByteLinkedOpenHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFF) & 0xFFFFFFFF;
      }
      if (this.next == -1) {
        ByteLinkedOpenHashSet.this.last = this.prev;
      } else {
        ByteLinkedOpenHashSet.this.link[this.next] ^= (ByteLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFF) << 32) & 0x0;
      }
      int pos = this.curr;
      int last;
      for (;;)
      {
        for (pos = (last = pos) + 1 & ByteLinkedOpenHashSet.this.mask; ByteLinkedOpenHashSet.this.used[pos] != 0; pos = pos + 1 & ByteLinkedOpenHashSet.this.mask)
        {
          int slot = HashCommon.murmurHash3(ByteLinkedOpenHashSet.this.key[pos]) & ByteLinkedOpenHashSet.this.mask;
          if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) {
            break;
          }
        }
        if (ByteLinkedOpenHashSet.this.used[pos] == 0) {
          break;
        }
        ByteLinkedOpenHashSet.this.key[last] = ByteLinkedOpenHashSet.this.key[pos];
        if (this.next == pos) {
          this.next = last;
        }
        if (this.prev == pos) {
      ...
      ByteLinkedOpenHashSet.this.used[last] = false;
      this.curr = -1;
    }
  }
}
