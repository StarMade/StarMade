...
{
  public static int downHeap(byte[] heap, int size, int local_i, ByteComparator local_c)
  {
    if (local_i >= size) {
      throw new IllegalArgumentException("Heap position (" + local_i + ") is larger than or equal to heap size (" + size + ")");
    }
    byte local_e = heap[local_i];
    if (local_c == null)
    {
      int child;
  ...
  public static int upHeap(byte[] heap, int size, int local_i, ByteComparator local_c)
  {
    if (local_i >= size) {
      throw new IllegalArgumentException("Heap position (" + local_i + ") is larger than or equal to heap size (" + size + ")");
    }
  ...
  public static void makeHeap(byte[] heap, int size, ByteComparator local_c)
  {
    int local_i = size / 2;
    while (local_i-- != 0) {
 ...
 * Qualified Name:     it.unimi.dsi.fastutil.bytes.ByteHeaps
