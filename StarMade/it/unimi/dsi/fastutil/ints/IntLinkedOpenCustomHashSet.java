  ...
  implements Serializable, Cloneable, Hash
{
  ...
  public IntLinkedOpenCustomHashSet(int expected, float local_f, IntHash.Strategy strategy)
  {
    ...
    this.link = new long[this.field_321];
  }
  ...
  public IntLinkedOpenCustomHashSet(Collection<? extends Integer> local_c, float local_f, IntHash.Strategy strategy)
  {
    this(local_c.size(), local_f, strategy);
    addAll(local_c);
  }
  
  public IntLinkedOpenCustomHashSet(Collection<? extends Integer> local_c, IntHash.Strategy strategy)
  {
    this(local_c, 0.75F, strategy);
  }
  
  public IntLinkedOpenCustomHashSet(IntCollection local_c, float local_f, IntHash.Strategy strategy)
  {
    this(local_c.size(), local_f, strategy);
    addAll(local_c);
  }
  
  public IntLinkedOpenCustomHashSet(IntCollection local_c, IntHash.Strategy strategy)
  {
    this(local_c, 0.75F, strategy);
  }
  
  public IntLinkedOpenCustomHashSet(IntIterator local_i, float local_f, IntHash.Strategy strategy)
  {
  ...
  public IntLinkedOpenCustomHashSet(IntIterator local_i, IntHash.Strategy strategy)
  {
    this(local_i, 0.75F, strategy);
  }
  
  public IntLinkedOpenCustomHashSet(Iterator<?> local_i, float local_f, IntHash.Strategy strategy)
  {
    this(IntIterators.asIntIterator(local_i), local_f, strategy);
  }
  
  public IntLinkedOpenCustomHashSet(Iterator<?> local_i, IntHash.Strategy strategy)
  {
    this(IntIterators.asIntIterator(local_i), strategy);
  }
  
  public IntLinkedOpenCustomHashSet(int[] local_a, int offset, int length, float local_f, IntHash.Strategy strategy)
  {
  ...
  public IntLinkedOpenCustomHashSet(int[] local_a, int offset, int length, IntHash.Strategy strategy)
  {
    this(local_a, offset, length, 0.75F, strategy);
  }
  
  public IntLinkedOpenCustomHashSet(int[] local_a, float local_f, IntHash.Strategy strategy)
  {
    this(local_a, 0, local_a.length, local_f, strategy);
  }
  
  public IntLinkedOpenCustomHashSet(int[] local_a, IntHash.Strategy strategy)
  {
  ...
  public boolean addAndMoveToFirst(int local_k)
  {
    int[] key = this.key;
    ...
    if (++this.size >= this.maxFill) {
      rehash(HashCommon.arraySize(this.size, this.field_320));
    }
    return true;
  }
  
  public boolean addAndMoveToLast(int local_k)
  {
    int[] key = this.key;
    boolean[] used = this.used;
    int mask = this.mask;
  ...
  {
    IntLinkedOpenCustomHashSet local_c;
    try
    {
      local_c = (IntLinkedOpenCustomHashSet)super.clone();
    }
    ...
    throws IOException, ClassNotFoundException
  {
    ...
    long[] link = this.link = new long[this.field_321];
    int prev = -1;
    this.first = (this.last = -1);
    ...
    extends AbstractIntListIterator
  {
    ...
    SetIterator(int from)
    {
      ...
      {
        for (int pos = HashCommon.murmurHash3(IntLinkedOpenCustomHashSet.this.strategy.hashCode(from)) & IntLinkedOpenCustomHashSet.this.mask; IntLinkedOpenCustomHashSet.this.used[pos] != 0; pos = pos + 1 & IntLinkedOpenCustomHashSet.this.mask) {
          if (IntLinkedOpenCustomHashSet.this.strategy.equals(IntLinkedOpenCustomHashSet.this.key[pos], from))
          {
    ...
    public void remove()
    {
      ...
      IntLinkedOpenCustomHashSet.this.size -= 1;
      if (this.prev == -1) {
        IntLinkedOpenCustomHashSet.this.first = this.next;
      } else {
        IntLinkedOpenCustomHashSet.this.link[this.prev] ^= (IntLinkedOpenCustomHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFF) & 0xFFFFFFFF;
      }
      if (this.next == -1) {
        IntLinkedOpenCustomHashSet.this.last = this.prev;
      } else {
        IntLinkedOpenCustomHashSet.this.link[this.next] ^= (IntLinkedOpenCustomHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFF) << 32) & 0x0;
      }
      int pos = this.curr;
      int last;
      for (;;)
      {
        for (pos = (last = pos) + 1 & IntLinkedOpenCustomHashSet.this.mask; IntLinkedOpenCustomHashSet.this.used[pos] != 0; pos = pos + 1 & IntLinkedOpenCustomHashSet.this.mask)
        {
          int slot = HashCommon.murmurHash3(IntLinkedOpenCustomHashSet.this.strategy.hashCode(IntLinkedOpenCustomHashSet.this.key[pos])) & IntLinkedOpenCustomHashSet.this.mask;
          if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) {
            break;
          }
        }
        if (IntLinkedOpenCustomHashSet.this.used[pos] == 0) {
          break;
        }
        IntLinkedOpenCustomHashSet.this.key[last] = IntLinkedOpenCustomHashSet.this.key[pos];
        if (this.next == pos) {
          this.next = last;
        }
        if (this.prev == pos) {
      ...
      IntLinkedOpenCustomHashSet.this.used[last] = false;
      this.curr = -1;
    }
  }
}
