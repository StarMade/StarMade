...
/*     */ 
/*     */ public class IntLinkedOpenCustomHashSet extends AbstractIntSortedSet
...
/* 194 */     while (i.hasNext()) add(i.nextInt());
/*     */   }
/*     */ 
...
/*     */   {
/* 211 */     this(IntIterators.asIntIterator(i), f, strategy);
/*     */   }
/*     */ 
/*     */   public IntLinkedOpenCustomHashSet(Iterator<?> i, IntHash.Strategy strategy)
/*     */   {
/* 219 */     this(IntIterators.asIntIterator(i), strategy);
/*     */   }
/*     */ 
...
/*     */   }
/* 609 */   public IntSortedSet tailSet(int from) { throw new UnsupportedOperationException(); } 
/* 610 */   public IntSortedSet headSet(int to) { throw new UnsupportedOperationException(); } 
/* 611 */   public IntSortedSet subSet(int from, int to) { throw new UnsupportedOperationException(); } 
/* 612 */   public IntComparator comparator() { return null; }
/*     */ 
/*     */ 
...
/*     */ 
/* 825 */     int[] key = this.key;
/* 826 */     int newMask = newN - 1;
/* 827 */     int[] newKey = new int[newN];
...
/*     */     try
/*     */     {
/* 874 */       c = (IntLinkedOpenCustomHashSet)super.clone();
/*     */     }
/*     */     catch (CloneNotSupportedException cantHappen) {
/* 877 */       throw new InternalError();
/*     */     }
/* 879 */     c.key = ((int[])this.key.clone());
...
/*     */     }
/* 901 */     return h; } 
/* 904 */   private void writeObject(ObjectOutputStream s) throws IOException { IntIterator i = iterator();
/* 905 */     s.defaultWriteObject();
/* 906 */     for (int j = this.size; j-- != 0; s.writeInt(i.nextInt()));
/*     */   }
/*     */ 
...
/* 914 */     int[] key = this.key = new int[this.n];
...
/* 627 */     int index = -1;
/*     */ 
/* 629 */     SetIterator() { this.next = IntLinkedOpenCustomHashSet.this.first;
/* 630 */       this.index = 0; }
/*     */ 
/*     */     SetIterator(int from) {
/* 633 */       if (IntLinkedOpenCustomHashSet.this.strategy.equals(IntLinkedOpenCustomHashSet.this.key[IntLinkedOpenCustomHashSet.this.last], from)) {
/* 634 */         this.prev = IntLinkedOpenCustomHashSet.this.last;
/* 635 */         this.index = IntLinkedOpenCustomHashSet.this.size;
/*     */       }
/*     */       else
/*     */       {
/* 639 */         int pos = HashCommon.murmurHash3(IntLinkedOpenCustomHashSet.this.strategy.hashCode(from)) & IntLinkedOpenCustomHashSet.this.mask;
/*     */ 
/* 641 */         while (IntLinkedOpenCustomHashSet.this.used[pos] != 0) {
/* 642 */           if (IntLinkedOpenCustomHashSet.this.strategy.equals(IntLinkedOpenCustomHashSet.this.key[pos], from))
/*     */           {
/* 644 */             this.next = ((int)IntLinkedOpenCustomHashSet.this.link[pos]);
/* 645 */             this.prev = pos;
/* 646 */             return;
/*     */           }
/* 648 */           pos = pos + 1 & IntLinkedOpenCustomHashSet.this.mask;
/*     */         }
...
/* 658 */       this.next = ((int)IntLinkedOpenCustomHashSet.this.link[this.curr]);
/* 659 */       this.prev = this.curr;
/* 660 */       if (this.index >= 0) this.index += 1;
/*     */ 
/* 662 */       return IntLinkedOpenCustomHashSet.this.key[this.curr];
/*     */     }
/*     */     public int previousInt() {
/* 665 */       if (!hasPrevious()) throw new NoSuchElementException();
/* 666 */       this.curr = this.prev;
/* 667 */       this.prev = ((int)(IntLinkedOpenCustomHashSet.this.link[this.curr] >>> 32));
/* 668 */       this.next = this.curr;
/* 669 */       if (this.index >= 0) this.index -= 1;
/* 670 */       return IntLinkedOpenCustomHashSet.this.key[this.curr];
/*     */     }
...
/*     */       }
/* 678 */       if (this.next == -1) {
/* 679 */         this.index = IntLinkedOpenCustomHashSet.this.size;
/* 680 */         return;
/*     */       }
/* 682 */       int pos = IntLinkedOpenCustomHashSet.this.first;
/* 683 */       this.index = 1;
/* 684 */       while (pos != this.prev) {
/* 685 */         pos = (int)IntLinkedOpenCustomHashSet.this.link[pos];
/* 686 */         this.index += 1;
/*     */       }
/*     */     }
/*     */ 
...
/*     */       {
/* 704 */         this.index -= 1;
/* 705 */         this.prev = ((int)(IntLinkedOpenCustomHashSet.this.link[this.curr] >>> 32));
/*     */       }
/*     */       else {
/* 708 */         this.next = ((int)IntLinkedOpenCustomHashSet.this.link[this.curr]);
/* 709 */       }IntLinkedOpenCustomHashSet.this.size -= 1;
/*     */ 
/* 712 */       if (this.prev == -1) IntLinkedOpenCustomHashSet.this.first = this.next;
/*     */       else
/* 714 */         IntLinkedOpenCustomHashSet.this.link[this.prev] ^= (IntLinkedOpenCustomHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFF) & 0xFFFFFFFF;
/* 715 */       if (this.next == -1) IntLinkedOpenCustomHashSet.this.last = this.prev;
/*     */       else
/* 717 */         IntLinkedOpenCustomHashSet.this.link[this.next] ^= (IntLinkedOpenCustomHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFF) << 32) & 0x0; int pos = this.curr;
/*     */       int last;
/*     */       while (true) {
/* 721 */         pos = (last = pos) + 1 & IntLinkedOpenCustomHashSet.this.mask;
/* 722 */         while (IntLinkedOpenCustomHashSet.this.used[pos] != 0) {
/* 723 */           int slot = HashCommon.murmurHash3(IntLinkedOpenCustomHashSet.this.strategy.hashCode(IntLinkedOpenCustomHashSet.this.key[pos])) & IntLinkedOpenCustomHashSet.this.mask;
/* 724 */           if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) break;
/* 725 */           pos = pos + 1 & IntLinkedOpenCustomHashSet.this.mask;
/*     */         }
/* 727 */         if (IntLinkedOpenCustomHashSet.this.used[pos] == 0) break;
/* 728 */         IntLinkedOpenCustomHashSet.this.key[last] = IntLinkedOpenCustomHashSet.this.key[pos];
/* 729 */         if (this.next == pos) this.next = last;
/* 730 */         if (this.prev == pos) this.prev = last;
/* 731 */         IntLinkedOpenCustomHashSet.this.fixPointers(pos, last);
/*     */       }
/* 733 */       IntLinkedOpenCustomHashSet.this.used[last] = false;
/* 734 */       this.curr = -1;
/*     */     }
/*     */   }
/*     */ }
