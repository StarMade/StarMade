  ...
  implements Serializable, Cloneable, Hash
{
  ...
  public IntLinkedOpenHashSet(int expected, float local_f)
  {
  ...
  public IntLinkedOpenHashSet(Collection<? extends Integer> local_c, float local_f)
  {
    this(local_c.size(), local_f);
    addAll(local_c);
  }
  
  public IntLinkedOpenHashSet(Collection<? extends Integer> local_c)
  {
    this(local_c, 0.75F);
  }
  
  public IntLinkedOpenHashSet(IntCollection local_c, float local_f)
  {
    this(local_c.size(), local_f);
    addAll(local_c);
  }
  
  public IntLinkedOpenHashSet(IntCollection local_c)
  {
    this(local_c, 0.75F);
  }
  
  public IntLinkedOpenHashSet(IntIterator local_i, float local_f)
  {
    this(16, local_f);
  ...
  public IntLinkedOpenHashSet(IntIterator local_i)
  {
    this(local_i, 0.75F);
  }
  
  public IntLinkedOpenHashSet(Iterator<?> local_i, float local_f)
  {
    this(IntIterators.asIntIterator(local_i), local_f);
  }
  
  public IntLinkedOpenHashSet(Iterator<?> local_i)
  {
    this(IntIterators.asIntIterator(local_i));
  }
  
  public IntLinkedOpenHashSet(int[] local_a, int offset, int length, float local_f)
  {
  ...
  public IntLinkedOpenHashSet(int[] local_a, int offset, int length)
  {
    this(local_a, offset, length, 0.75F);
  }
  
  public IntLinkedOpenHashSet(int[] local_a, float local_f)
  {
    this(local_a, 0, local_a.length, local_f);
  }
  
  public IntLinkedOpenHashSet(int[] local_a)
  {
    this(local_a, 0.75F);
  }
  ...
  {
    IntLinkedOpenHashSet local_c;
    try
    {
      local_c = (IntLinkedOpenHashSet)super.clone();
    }
    ...
    extends AbstractIntListIterator
  {
    ...
    SetIterator(int from)
    {
      ...
      {
        for (int pos = HashCommon.murmurHash3(from) & IntLinkedOpenHashSet.this.mask; IntLinkedOpenHashSet.this.used[pos] != 0; pos = pos + 1 & IntLinkedOpenHashSet.this.mask) {
          if (IntLinkedOpenHashSet.this.key[pos] == from)
          {
    ...
    public void remove()
    {
      ...
      IntLinkedOpenHashSet.this.size -= 1;
      if (this.prev == -1) {
        IntLinkedOpenHashSet.this.first = this.next;
      } else {
        IntLinkedOpenHashSet.this.link[this.prev] ^= (IntLinkedOpenHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFF) & 0xFFFFFFFF;
      }
      if (this.next == -1) {
        IntLinkedOpenHashSet.this.last = this.prev;
      } else {
        IntLinkedOpenHashSet.this.link[this.next] ^= (IntLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFF) << 32) & 0x0;
      }
      int pos = this.curr;
      int last;
      for (;;)
      {
        for (pos = (last = pos) + 1 & IntLinkedOpenHashSet.this.mask; IntLinkedOpenHashSet.this.used[pos] != 0; pos = pos + 1 & IntLinkedOpenHashSet.this.mask)
        {
          int slot = HashCommon.murmurHash3(IntLinkedOpenHashSet.this.key[pos]) & IntLinkedOpenHashSet.this.mask;
          if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) {
            break;
          }
        }
        if (IntLinkedOpenHashSet.this.used[pos] == 0) {
          break;
        }
        IntLinkedOpenHashSet.this.key[last] = IntLinkedOpenHashSet.this.key[pos];
        if (this.next == pos) {
          this.next = last;
        }
        if (this.prev == pos) {
      ...
      IntLinkedOpenHashSet.this.used[last] = false;
      this.curr = -1;
    }
  }
}
