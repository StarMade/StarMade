...
{
  public static int downHeap(int[] heap, int size, int local_i, IntComparator local_c)
  {
    if (local_i >= size) {
  ...
  public static int upHeap(int[] heap, int size, int local_i, IntComparator local_c)
  {
    if (local_i >= size) {
      throw new IllegalArgumentException("Heap position (" + local_i + ") is larger than or equal to heap size (" + size + ")");
    }
  ...
  public static void makeHeap(int[] heap, int size, IntComparator local_c)
  {
    int local_i = size / 2;
    while (local_i-- != 0) {
