  ...
  implements Serializable, Cloneable, Hash
{
  ...
  public ObjectOpenHashSet(int expected, float local_f)
  {
  ...
  public ObjectOpenHashSet(Collection<? extends K> local_c, float local_f)
  {
    this(local_c.size(), local_f);
    addAll(local_c);
  }
  
  public ObjectOpenHashSet(Collection<? extends K> local_c)
  {
    this(local_c, 0.75F);
  }
  
  public ObjectOpenHashSet(ObjectCollection<? extends K> local_c, float local_f)
  {
    this(local_c.size(), local_f);
    addAll(local_c);
  }
  
  public ObjectOpenHashSet(ObjectCollection<? extends K> local_c)
  {
    this(local_c, 0.75F);
  }
  
  public ObjectOpenHashSet(ObjectIterator<K> local_i, float local_f)
  {
    this(16, local_f);
  ...
  public ObjectOpenHashSet(ObjectIterator<K> local_i)
  {
    this(local_i, 0.75F);
  }
  
  public ObjectOpenHashSet(K[] local_a, int offset, int length, float local_f)
  {
  ...
  public ObjectOpenHashSet(K[] local_a, int offset, int length)
  {
    this(local_a, offset, length, 0.75F);
  }
  
  public ObjectOpenHashSet(K[] local_a, float local_f)
  {
    this(local_a, 0, local_a.length, local_f);
  }
  
  public ObjectOpenHashSet(K[] local_a)
  {
    this(local_a, 0.75F);
  }
  ...
  {
    ObjectOpenHashSet<K> local_c;
    try
    {
      local_c = (ObjectOpenHashSet)super.clone();
    }
  ...
  {
    int pos = ObjectOpenHashSet.this.field_74;
    int last = -1;
    int field_333 = ObjectOpenHashSet.this.size;
    ...
    public K next()
    {
      ...
      K retVal = ObjectOpenHashSet.this.key[(this.last = this.pos)];
      if (this.field_333 != 0)
      {
        boolean[] used = ObjectOpenHashSet.this.used;
    ...
    final int shiftKeys(int pos)
    {
      ...
      for (;;)
      {
        for (pos = (last = pos) + 1 & ObjectOpenHashSet.this.mask; ObjectOpenHashSet.this.used[pos] != 0; pos = pos + 1 & ObjectOpenHashSet.this.mask)
        {
          int slot = (ObjectOpenHashSet.this.key[pos] == null ? 142593372 : HashCommon.murmurHash3(ObjectOpenHashSet.this.key[pos].hashCode())) & ObjectOpenHashSet.this.mask;
          if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) {
            break;
          }
        }
        if (ObjectOpenHashSet.this.used[pos] == 0) {
          break;
        }
        if (pos < last)
        {
          ...
          this.wrapped.add(ObjectOpenHashSet.this.key[pos]);
        }
        ObjectOpenHashSet.this.key[last] = ObjectOpenHashSet.this.key[pos];
      }
      ObjectOpenHashSet.this.used[last] = false;
      ObjectOpenHashSet.this.key[last] = null;
      return last;
    }
