/*     */ package it.unimi.dsi.fastutil.objects;
/*     */ 
/*     */ import it.unimi.dsi.fastutil.Stack;
/*     */ import java.io.Serializable;
/*     */ import java.util.Collection;
/*     */ import java.util.Iterator;
/*     */ import java.util.List;
/*     */ import java.util.ListIterator;
/*     */ import java.util.NoSuchElementException;
/*     */ 
/*     */ public abstract class AbstractObjectList<K> extends AbstractObjectCollection<K>
/*     */   implements ObjectList<K>, Stack<K>
/*     */ {
/*     */   protected void ensureIndex(int index)
/*     */   {
/*  62 */     if (index < 0) throw new IndexOutOfBoundsException(new StringBuilder().append("Index (").append(index).append(") is negative").toString());
/*  63 */     if (index > size()) throw new IndexOutOfBoundsException(new StringBuilder().append("Index (").append(index).append(") is greater than list size (").append(size()).append(")").toString());
/*     */   }
/*     */ 
/*     */   protected void ensureRestrictedIndex(int index)
/*     */   {
/*  71 */     if (index < 0) throw new IndexOutOfBoundsException(new StringBuilder().append("Index (").append(index).append(") is negative").toString());
/*  72 */     if (index >= size()) throw new IndexOutOfBoundsException(new StringBuilder().append("Index (").append(index).append(") is greater than or equal to list size (").append(size()).append(")").toString()); 
/*     */   }
/*     */ 
/*  75 */   public void add(int index, K k) { throw new UnsupportedOperationException(); }
/*     */ 
/*     */   public boolean add(K k) {
/*  78 */     add(size(), k);
/*  79 */     return true;
/*     */   }
/*     */   public K remove(int i) {
/*  82 */     throw new UnsupportedOperationException();
/*     */   }
/*     */   public K set(int index, K k) {
/*  85 */     throw new UnsupportedOperationException();
/*     */   }
/*     */   public boolean addAll(int index, Collection<? extends K> c) {
/*  88 */     ensureIndex(index);
/*  89 */     int n = c.size();
/*  90 */     if (n == 0) return false;
/*  91 */     Iterator i = c.iterator();
/*  92 */     while (n-- != 0) add(index++, i.next());
/*  93 */     return true;
/*     */   }
/*     */ 
/*     */   public boolean addAll(Collection<? extends K> c) {
/*  97 */     return addAll(size(), c);
/*     */   }
/*     */ 
/*     */   @Deprecated
/*     */   public ObjectListIterator<K> objectListIterator() {
/* 102 */     return listIterator();
/*     */   }
/*     */ 
/*     */   @Deprecated
/*     */   public ObjectListIterator<K> objectListIterator(int index) {
/* 107 */     return listIterator(index);
/*     */   }
/*     */   public ObjectListIterator<K> iterator() {
/* 110 */     return listIterator();
/*     */   }
/*     */   public ObjectListIterator<K> listIterator() {
/* 113 */     return listIterator(0);
/*     */   }
/*     */   public ObjectListIterator<K> listIterator(final int index) {
/* 116 */     return new AbstractObjectListIterator() {
/* 117 */       int pos = index; int last = -1;
/*     */ 
/* 118 */       public boolean hasNext() { return this.pos < AbstractObjectList.this.size(); } 
/* 119 */       public boolean hasPrevious() { return this.pos > 0; } 
/* 120 */       public K next() { if (!hasNext()) throw new NoSuchElementException(); return AbstractObjectList.this.get(this.last = this.pos++); } 
/* 121 */       public K previous() { if (!hasPrevious()) throw new NoSuchElementException(); return AbstractObjectList.this.get(this.last = --this.pos); } 
/* 122 */       public int nextIndex() { return this.pos; } 
/* 123 */       public int previousIndex() { return this.pos - 1; } 
/*     */       public void add(K k) {
/* 125 */         if (this.last == -1) throw new IllegalStateException();
/* 126 */         AbstractObjectList.this.add(this.pos++, k);
/* 127 */         this.last = -1;
/*     */       }
/*     */       public void set(K k) {
/* 130 */         if (this.last == -1) throw new IllegalStateException();
/* 131 */         AbstractObjectList.this.set(this.last, k);
/*     */       }
/*     */       public void remove() {
/* 134 */         if (this.last == -1) throw new IllegalStateException();
/* 135 */         AbstractObjectList.this.remove(this.last);
/*     */ 
/* 137 */         if (this.last < this.pos) this.pos -= 1;
/* 138 */         this.last = -1;
/*     */       }
/*     */     };
/*     */   }
/*     */ 
/*     */   public boolean contains(Object k)
/*     */   {
/* 146 */     return indexOf(k) >= 0;
/*     */   }
/*     */ 
/*     */   public int indexOf(Object k) {
/* 150 */     ObjectListIterator i = listIterator();
/*     */ 
/* 154 */     for (; i.hasNext(); 
/* 154 */       return i.previousIndex())
/*     */     {
/* 153 */       label5: Object e = i.next();
/* 154 */       if (k == null ? e != null : !k.equals(e)) break label5;
/*     */     }
/* 156 */     return -1;
/*     */   }
/*     */ 
/*     */   public int lastIndexOf(Object k) {
/* 160 */     ObjectListIterator i = listIterator(size());
/*     */ 
/* 164 */     for (; i.hasPrevious(); 
/* 164 */       return i.nextIndex())
/*     */     {
/* 163 */       label9: Object e = i.previous();
/* 164 */       if (k == null ? e != null : !k.equals(e)) break label9;
/*     */     }
/* 166 */     return -1;
/*     */   }
/*     */ 
/*     */   public void size(int size) {
/* 170 */     int i = size();
/* 171 */     for (size <= i; i++ < size; add(null));
/* 172 */     while (i-- != size) remove(i);
/*     */   }
/*     */ 
/*     */   public ObjectList<K> subList(int from, int to)
/*     */   {
/* 177 */     ensureIndex(from);
/* 178 */     ensureIndex(to);
/* 179 */     if (from > to) throw new IndexOutOfBoundsException(new StringBuilder().append("Start index (").append(from).append(") is greater than end index (").append(to).append(")").toString());
/*     */ 
/* 181 */     return new ObjectSubList(this, from, to);
/*     */   }
/*     */ 
/*     */   @Deprecated
/*     */   public ObjectList<K> objectSubList(int from, int to)
/*     */   {
/* 188 */     return subList(from, to);
/*     */   }
/*     */ 
/*     */   public void removeElements(int from, int to)
/*     */   {
/* 202 */     ensureIndex(to);
/* 203 */     ObjectListIterator i = listIterator(from);
/* 204 */     int n = to - from;
/* 205 */     if (n < 0) throw new IllegalArgumentException(new StringBuilder().append("Start index (").append(from).append(") is greater than end index (").append(to).append(")").toString());
/* 206 */     while (n-- != 0) {
/* 207 */       i.next();
/* 208 */       i.remove();
/*     */     }
/*     */   }
/*     */ 
/*     */   public void addElements(int index, K[] a, int offset, int length)
/*     */   {
/* 224 */     ensureIndex(index);
/* 225 */     if (offset < 0) throw new ArrayIndexOutOfBoundsException(new StringBuilder().append("Offset (").append(offset).append(") is negative").toString());
/* 226 */     if (offset + length > a.length) throw new ArrayIndexOutOfBoundsException(new StringBuilder().append("End index (").append(offset + length).append(") is greater than array length (").append(a.length).append(")").toString());
/* 227 */     while (length-- != 0) add(index++, a[(offset++)]); 
/*     */   }
/*     */ 
/*     */   public void addElements(int index, K[] a)
/*     */   {
/* 231 */     addElements(index, a, 0, a.length);
/*     */   }
/*     */ 
/*     */   public void getElements(int from, Object[] a, int offset, int length)
/*     */   {
/* 246 */     ObjectListIterator i = listIterator(from);
/* 247 */     if (offset < 0) throw new ArrayIndexOutOfBoundsException(new StringBuilder().append("Offset (").append(offset).append(") is negative").toString());
/* 248 */     if (offset + length > a.length) throw new ArrayIndexOutOfBoundsException(new StringBuilder().append("End index (").append(offset + length).append(") is greater than array length (").append(a.length).append(")").toString());
/* 249 */     if (from + length > size()) throw new IndexOutOfBoundsException(new StringBuilder().append("End index (").append(from + length).append(") is greater than list size (").append(size()).append(")").toString());
/* 250 */     while (length-- != 0) a[(offset++)] = i.next();
/*     */   }
/*     */ 
/*     */   private boolean valEquals(Object a, Object b)
/*     */   {
/* 255 */     return a == null ? false : b == null ? true : a.equals(b);
/*     */   }
/*     */ 
/*     */   public boolean equals(Object o)
/*     */   {
/* 260 */     if (o == this) return true;
/* 261 */     if (!(o instanceof List)) return false;
/* 262 */     List l = (List)o;
/* 263 */     int s = size();
/* 264 */     if (s != l.size()) return false;
/*     */ 
/* 266 */     ListIterator i1 = listIterator(); ListIterator i2 = l.listIterator();
/*     */ 
/* 271 */     while (s-- != 0) if (!valEquals(i1.next(), i2.next())) return false;
/*     */ 
/* 273 */     return true;
/*     */   }
/*     */ 
/*     */   public int compareTo(List<? extends K> l)
/*     */   {
/* 290 */     if (l == this) return 0;
/*     */ 
/* 292 */     if ((l instanceof ObjectList))
/*     */     {
/* 294 */       ObjectListIterator i1 = listIterator(); ObjectListIterator i2 = ((ObjectList)l).listIterator();
/*     */ 
/* 298 */       while ((i1.hasNext()) && (i2.hasNext())) {
/* 299 */         Object e1 = i1.next();
/* 300 */         Object e2 = i2.next();
/*     */         int r;
/* 301 */         if ((r = ((Comparable)e1).compareTo(e2)) != 0) return r;
/*     */       }
/* 303 */       return i1.hasNext() ? 1 : i2.hasNext() ? -1 : 0;
/*     */     }
/*     */ 
/* 306 */     ListIterator i1 = listIterator(); ListIterator i2 = l.listIterator();
/*     */ 
/* 309 */     while ((i1.hasNext()) && (i2.hasNext()))
/*     */     {
/* 310 */       int r;
/* 310 */       if ((r = ((Comparable)i1.next()).compareTo(i2.next())) != 0) return r;
/*     */     }
/* 312 */     return i1.hasNext() ? 1 : i2.hasNext() ? -1 : 0;
/*     */   }
/*     */ 
/*     */   public int hashCode()
/*     */   {
/* 321 */     ObjectIterator i = iterator();
/* 322 */     int h = 1; int s = size();
/* 323 */     while (s-- != 0) {
/* 324 */       Object k = i.next();
/* 325 */       h = 31 * h + (k == null ? 0 : k.hashCode());
/*     */     }
/* 327 */     return h;
/*     */   }
/*     */ 
/*     */   public void push(K o)
/*     */   {
/* 332 */     add(o);
/*     */   }
/*     */ 
/*     */   public K pop() {
/* 336 */     if (isEmpty()) throw new NoSuchElementException();
/* 337 */     return remove(size() - 1);
/*     */   }
/*     */ 
/*     */   public K top() {
/* 341 */     if (isEmpty()) throw new NoSuchElementException();
/* 342 */     return get(size() - 1);
/*     */   }
/*     */ 
/*     */   public K peek(int i) {
/* 346 */     return get(size() - 1 - i);
/*     */   }
/*     */   public String toString() {
/* 349 */     StringBuilder s = new StringBuilder();
/* 350 */     ObjectIterator i = iterator();
/* 351 */     int n = size();
/*     */ 
/* 353 */     boolean first = true;
/* 354 */     s.append("[");
/* 355 */     while (n-- != 0) {
/* 356 */       if (first) first = false; else
/* 357 */         s.append(", ");
/* 358 */       Object k = i.next();
/* 359 */       if (this == k) s.append("(this list)"); else
/* 360 */         s.append(String.valueOf(k));
/*     */     }
/* 362 */     s.append("]");
/* 363 */     return s.toString();
/*     */   }
/*     */   public static class ObjectSubList<K> extends AbstractObjectList<K> implements Serializable { public static final long serialVersionUID = -7046029254386353129L;
/*     */     protected final ObjectList<K> l;
/*     */     protected final int from;
/*     */     protected int to;
/*     */     private static final boolean ASSERTS = false;
/*     */ 
/* 375 */     public ObjectSubList(ObjectList<K> l, int from, int to) { this.l = l;
/* 376 */       this.from = from;
/* 377 */       this.to = to;
/*     */     }
/*     */ 
/*     */     private void assertRange()
/*     */     {
/*     */     }
/*     */ 
/*     */     public boolean add(K k)
/*     */     {
/* 387 */       this.l.add(this.to, k);
/* 388 */       this.to += 1;
/*     */ 
/* 390 */       return true;
/*     */     }
/*     */     public void add(int index, K k) {
/* 393 */       ensureIndex(index);
/* 394 */       this.l.add(this.from + index, k);
/* 395 */       this.to += 1;
/*     */     }
/*     */ 
/*     */     public boolean addAll(int index, Collection<? extends K> c) {
/* 399 */       ensureIndex(index);
/* 400 */       this.to += c.size();
/*     */ 
/* 406 */       return this.l.addAll(this.from + index, c);
/*     */     }
/*     */     public K get(int index) {
/* 409 */       ensureRestrictedIndex(index);
/* 410 */       return this.l.get(this.from + index);
/*     */     }
/*     */     public K remove(int index) {
/* 413 */       ensureRestrictedIndex(index);
/* 414 */       this.to -= 1;
/* 415 */       return this.l.remove(this.from + index);
/*     */     }
/*     */     public K set(int index, K k) {
/* 418 */       ensureRestrictedIndex(index);
/* 419 */       return this.l.set(this.from + index, k);
/*     */     }
/*     */     public void clear() {
/* 422 */       removeElements(0, size());
/*     */     }
/*     */ 
/*     */     public int size() {
/* 426 */       return this.to - this.from;
/*     */     }
/*     */     public void getElements(int from, Object[] a, int offset, int length) {
/* 429 */       ensureIndex(from);
/* 430 */       if (from + length > size()) throw new IndexOutOfBoundsException("End index (" + from + length + ") is greater than list size (" + size() + ")");
/* 431 */       this.l.getElements(this.from + from, a, offset, length);
/*     */     }
/*     */     public void removeElements(int from, int to) {
/* 434 */       ensureIndex(from);
/* 435 */       ensureIndex(to);
/* 436 */       this.l.removeElements(this.from + from, this.from + to);
/* 437 */       this.to -= to - from;
/*     */     }
/*     */ 
/*     */     public void addElements(int index, K[] a, int offset, int length) {
/* 441 */       ensureIndex(index);
/* 442 */       this.l.addElements(this.from + index, a, offset, length);
/* 443 */       this.to += length;
/*     */     }
/*     */ 
/*     */     public ObjectListIterator<K> listIterator(final int index) {
/* 447 */       ensureIndex(index);
/* 448 */       return new AbstractObjectListIterator() {
/* 449 */         int pos = index; int last = -1;
/*     */ 
/* 450 */         public boolean hasNext() { return this.pos < AbstractObjectList.ObjectSubList.this.size(); } 
/* 451 */         public boolean hasPrevious() { return this.pos > 0; } 
/* 452 */         public K next() { if (!hasNext()) throw new NoSuchElementException(); return AbstractObjectList.ObjectSubList.this.l.get(AbstractObjectList.ObjectSubList.this.from + (this.last = this.pos++)); } 
/* 453 */         public K previous() { if (!hasPrevious()) throw new NoSuchElementException(); return AbstractObjectList.ObjectSubList.this.l.get(AbstractObjectList.ObjectSubList.this.from + (this.last = --this.pos)); } 
/* 454 */         public int nextIndex() { return this.pos; } 
/* 455 */         public int previousIndex() { return this.pos - 1; } 
/*     */         public void add(K k) {
/* 457 */           if (this.last == -1) throw new IllegalStateException();
/* 458 */           AbstractObjectList.ObjectSubList.this.add(this.pos++, k);
/* 459 */           this.last = -1;
/*     */         }
/*     */ 
/*     */         public void set(K k) {
/* 463 */           if (this.last == -1) throw new IllegalStateException();
/* 464 */           AbstractObjectList.ObjectSubList.this.set(this.last, k);
/*     */         }
/*     */         public void remove() {
/* 467 */           if (this.last == -1) throw new IllegalStateException();
/* 468 */           AbstractObjectList.ObjectSubList.this.remove(this.last);
/*     */ 
/* 470 */           if (this.last < this.pos) this.pos -= 1;
/* 471 */           this.last = -1;
/*     */         }
/*     */       };
/*     */     }
/*     */ 
/*     */     public ObjectList<K> subList(int from, int to) {
/* 477 */       ensureIndex(from);
/* 478 */       ensureIndex(to);
/* 479 */       if (from > to) throw new IllegalArgumentException("Start index (" + from + ") is greater than end index (" + to + ")");
/* 480 */       return new ObjectSubList(this, from, to);
/*     */     }
/*     */ 
/*     */     public boolean remove(Object o) {
/* 484 */       int index = indexOf(o);
/* 485 */       if (index == -1) return false;
/* 486 */       remove(index);
/* 487 */       return true;
/*     */     }
/*     */   }
/*     */ }

/* Location:           C:\Users\Raul\Desktop\StarMade\StarMade.jar
 * Qualified Name:     it.unimi.dsi.fastutil.objects.AbstractObjectList
 * JD-Core Version:    0.6.2
 */