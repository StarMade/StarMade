  ...
  implements Serializable, Cloneable, Hash
{
  ...
  public ObjectLinkedOpenHashSet(int expected, float local_f)
  {
  ...
  public ObjectLinkedOpenHashSet(Collection<? extends K> local_c, float local_f)
  {
    this(local_c.size(), local_f);
    addAll(local_c);
  }
  
  public ObjectLinkedOpenHashSet(Collection<? extends K> local_c)
  {
    this(local_c, 0.75F);
  }
  
  public ObjectLinkedOpenHashSet(ObjectCollection<? extends K> local_c, float local_f)
  {
    this(local_c.size(), local_f);
    addAll(local_c);
  }
  
  public ObjectLinkedOpenHashSet(ObjectCollection<? extends K> local_c)
  {
    this(local_c, 0.75F);
  }
  
  public ObjectLinkedOpenHashSet(ObjectIterator<K> local_i, float local_f)
  {
    this(16, local_f);
  ...
  public ObjectLinkedOpenHashSet(ObjectIterator<K> local_i)
  {
    this(local_i, 0.75F);
  }
  
  public ObjectLinkedOpenHashSet(K[] local_a, int offset, int length, float local_f)
  {
  ...
  public ObjectLinkedOpenHashSet(K[] local_a, int offset, int length)
  {
    this(local_a, offset, length, 0.75F);
  }
  
  public ObjectLinkedOpenHashSet(K[] local_a, float local_f)
  {
    this(local_a, 0, local_a.length, local_f);
  }
  
  public ObjectLinkedOpenHashSet(K[] local_a)
  {
    this(local_a, 0.75F);
  }
  ...
  {
    ObjectLinkedOpenHashSet<K> local_c;
    try
    {
      local_c = (ObjectLinkedOpenHashSet)super.clone();
    }
  ...
  public int hashCode()
  {
    ...
    while (local_j-- != 0)
    {
      ...
      if (this != this.key[local_i]) {
        local_h += (this.key[local_i] == null ? 0 : this.key[local_i].hashCode());
      }
      local_i++;
    }
    return local_h;
    ...
    extends AbstractObjectListIterator<K>
  {
    ...
    SetIterator()
    {
      ...
      {
        for (int pos = (from == null ? 142593372 : HashCommon.murmurHash3(from.hashCode())) & ObjectLinkedOpenHashSet.this.mask; ObjectLinkedOpenHashSet.this.used[pos] != 0; pos = pos + 1 & ObjectLinkedOpenHashSet.this.mask) {
          if (ObjectLinkedOpenHashSet.this.key[pos] == null ? from == null : ObjectLinkedOpenHashSet.this.key[pos].equals(from))
          {
    ...
    public void remove()
    {
      ...
      ObjectLinkedOpenHashSet.this.size -= 1;
      if (this.prev == -1) {
        ObjectLinkedOpenHashSet.this.first = this.next;
      } else {
        ObjectLinkedOpenHashSet.this.link[this.prev] ^= (ObjectLinkedOpenHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFF) & 0xFFFFFFFF;
      }
      if (this.next == -1) {
        ObjectLinkedOpenHashSet.this.last = this.prev;
      } else {
        ObjectLinkedOpenHashSet.this.link[this.next] ^= (ObjectLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFF) << 32) & 0x0;
      }
      int pos = this.curr;
      int last;
      for (;;)
      {
        for (pos = (last = pos) + 1 & ObjectLinkedOpenHashSet.this.mask; ObjectLinkedOpenHashSet.this.used[pos] != 0; pos = pos + 1 & ObjectLinkedOpenHashSet.this.mask)
        {
          int slot = (ObjectLinkedOpenHashSet.this.key[pos] == null ? 142593372 : HashCommon.murmurHash3(ObjectLinkedOpenHashSet.this.key[pos].hashCode())) & ObjectLinkedOpenHashSet.this.mask;
          if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) {
            break;
          }
        }
        if (ObjectLinkedOpenHashSet.this.used[pos] == 0) {
          break;
        }
        ObjectLinkedOpenHashSet.this.key[last] = ObjectLinkedOpenHashSet.this.key[pos];
        if (this.next == pos) {
          this.next = last;
        }
        if (this.prev == pos) {
      ...
      ObjectLinkedOpenHashSet.this.used[last] = false;
      ObjectLinkedOpenHashSet.this.key[last] = null;
      this.curr = -1;
    }
  }
}
