...
{
  public static int downHeap(short[] heap, int size, int local_i, ShortComparator local_c)
  {
    if (local_i >= size) {
      throw new IllegalArgumentException("Heap position (" + local_i + ") is larger than or equal to heap size (" + size + ")");
    }
    short local_e = heap[local_i];
    if (local_c == null)
    {
      int child;
  ...
  public static int upHeap(short[] heap, int size, int local_i, ShortComparator local_c)
  {
    if (local_i >= size) {
      throw new IllegalArgumentException("Heap position (" + local_i + ") is larger than or equal to heap size (" + size + ")");
    }
  ...
  public static void makeHeap(short[] heap, int size, ShortComparator local_c)
  {
    int local_i = size / 2;
    while (local_i-- != 0) {
