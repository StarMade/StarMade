  ...
  implements Serializable, Cloneable, Hash
{
  ...
  public ShortLinkedOpenHashSet(int expected, float local_f)
  {
  ...
  public ShortLinkedOpenHashSet(Collection<? extends Short> local_c, float local_f)
  {
    this(local_c.size(), local_f);
    addAll(local_c);
  }
  
  public ShortLinkedOpenHashSet(Collection<? extends Short> local_c)
  {
    this(local_c, 0.75F);
  }
  
  public ShortLinkedOpenHashSet(ShortCollection local_c, float local_f)
  {
    this(local_c.size(), local_f);
    addAll(local_c);
  }
  
  public ShortLinkedOpenHashSet(ShortCollection local_c)
  {
    this(local_c, 0.75F);
  }
  
  public ShortLinkedOpenHashSet(ShortIterator local_i, float local_f)
  {
    this(16, local_f);
  ...
  public ShortLinkedOpenHashSet(ShortIterator local_i)
  {
    this(local_i, 0.75F);
  }
  
  public ShortLinkedOpenHashSet(Iterator<?> local_i, float local_f)
  {
    this(ShortIterators.asShortIterator(local_i), local_f);
  }
  
  public ShortLinkedOpenHashSet(Iterator<?> local_i)
  {
    this(ShortIterators.asShortIterator(local_i));
  }
  
  public ShortLinkedOpenHashSet(short[] local_a, int offset, int length, float local_f)
  {
  ...
  public ShortLinkedOpenHashSet(short[] local_a, int offset, int length)
  {
    this(local_a, offset, length, 0.75F);
  }
  
  public ShortLinkedOpenHashSet(short[] local_a, float local_f)
  {
    this(local_a, 0, local_a.length, local_f);
  }
  
  public ShortLinkedOpenHashSet(short[] local_a)
  {
    this(local_a, 0.75F);
  }
  ...
  {
    ShortLinkedOpenHashSet local_c;
    try
    {
      local_c = (ShortLinkedOpenHashSet)super.clone();
    }
    ...
    extends AbstractShortListIterator
  {
    ...
    SetIterator(short from)
    {
      ...
      {
        for (int pos = HashCommon.murmurHash3(from) & ShortLinkedOpenHashSet.this.mask; ShortLinkedOpenHashSet.this.used[pos] != 0; pos = pos + 1 & ShortLinkedOpenHashSet.this.mask) {
          if (ShortLinkedOpenHashSet.this.key[pos] == from)
          {
    ...
    public void remove()
    {
      ...
      ShortLinkedOpenHashSet.this.size -= 1;
      if (this.prev == -1) {
        ShortLinkedOpenHashSet.this.first = this.next;
      } else {
        ShortLinkedOpenHashSet.this.link[this.prev] ^= (ShortLinkedOpenHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFF) & 0xFFFFFFFF;
      }
      if (this.next == -1) {
        ShortLinkedOpenHashSet.this.last = this.prev;
      } else {
        ShortLinkedOpenHashSet.this.link[this.next] ^= (ShortLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFF) << 32) & 0x0;
      }
      int pos = this.curr;
      int last;
      for (;;)
      {
        for (pos = (last = pos) + 1 & ShortLinkedOpenHashSet.this.mask; ShortLinkedOpenHashSet.this.used[pos] != 0; pos = pos + 1 & ShortLinkedOpenHashSet.this.mask)
        {
          int slot = HashCommon.murmurHash3(ShortLinkedOpenHashSet.this.key[pos]) & ShortLinkedOpenHashSet.this.mask;
          if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) {
            break;
          }
        }
        if (ShortLinkedOpenHashSet.this.used[pos] == 0) {
          break;
        }
        ShortLinkedOpenHashSet.this.key[last] = ShortLinkedOpenHashSet.this.key[pos];
        if (this.next == pos) {
          this.next = last;
        }
        if (this.prev == pos) {
      ...
      ShortLinkedOpenHashSet.this.used[last] = false;
      this.curr = -1;
    }
  }
}
