...
/*     */ 
/*     */ public class ShortLinkedOpenHashSet extends AbstractShortSortedSet
...
/*     */   {
/* 182 */     this(16, f);
/* 183 */     while (i.hasNext()) add(i.nextShort());
/*     */   }
/*     */ 
...
/*     */   {
/* 198 */     this(ShortIterators.asShortIterator(i), f);
/*     */   }
/*     */ 
/*     */   public ShortLinkedOpenHashSet(Iterator<?> i)
/*     */   {
/* 205 */     this(ShortIterators.asShortIterator(i));
/*     */   }
/*     */ 
...
/*     */   }
/* 584 */   public ShortSortedSet tailSet(short from) { throw new UnsupportedOperationException(); } 
/* 585 */   public ShortSortedSet headSet(short to) { throw new UnsupportedOperationException(); } 
/* 586 */   public ShortSortedSet subSet(short from, short to) { throw new UnsupportedOperationException(); } 
/* 587 */   public ShortComparator comparator() { return null; }
/*     */ 
/*     */ 
...
/* 802 */     short[] newKey = new short[newN];
...
/*     */     try
/*     */     {
/* 849 */       c = (ShortLinkedOpenHashSet)super.clone();
/*     */     }
/*     */     catch (CloneNotSupportedException cantHappen) {
/* 852 */       throw new InternalError();
/*     */     }
/* 854 */     c.key = ((short[])this.key.clone());
...
/*     */     }
/* 875 */     return h; } 
/* 878 */   private void writeObject(ObjectOutputStream s) throws IOException { ShortIterator i = iterator();
/* 879 */     s.defaultWriteObject();
/* 880 */     for (int j = this.size; j-- != 0; s.writeShort(i.nextShort()));
/*     */   }
/*     */ 
...
/* 888 */     short[] key = this.key = new short[this.n];
...
/* 602 */     int index = -1;
/*     */ 
/* 604 */     SetIterator() { this.next = ShortLinkedOpenHashSet.this.first;
/* 605 */       this.index = 0; }
/*     */ 
/*     */     SetIterator(short from) {
/* 608 */       if (ShortLinkedOpenHashSet.this.key[ShortLinkedOpenHashSet.this.last] == from) {
/* 609 */         this.prev = ShortLinkedOpenHashSet.this.last;
/* 610 */         this.index = ShortLinkedOpenHashSet.this.size;
/*     */       }
/*     */       else
/*     */       {
/* 614 */         int pos = HashCommon.murmurHash3(from) & ShortLinkedOpenHashSet.this.mask;
/*     */ 
/* 616 */         while (ShortLinkedOpenHashSet.this.used[pos] != 0) {
/* 617 */           if (ShortLinkedOpenHashSet.this.key[pos] == from)
/*     */           {
/* 619 */             this.next = ((int)ShortLinkedOpenHashSet.this.link[pos]);
/* 620 */             this.prev = pos;
/* 621 */             return;
/*     */           }
/* 623 */           pos = pos + 1 & ShortLinkedOpenHashSet.this.mask;
/*     */         }
...
/* 633 */       this.next = ((int)ShortLinkedOpenHashSet.this.link[this.curr]);
/* 634 */       this.prev = this.curr;
/* 635 */       if (this.index >= 0) this.index += 1;
/*     */ 
/* 637 */       return ShortLinkedOpenHashSet.this.key[this.curr];
/*     */     }
/*     */     public short previousShort() {
/* 640 */       if (!hasPrevious()) throw new NoSuchElementException();
/* 641 */       this.curr = this.prev;
/* 642 */       this.prev = ((int)(ShortLinkedOpenHashSet.this.link[this.curr] >>> 32));
/* 643 */       this.next = this.curr;
/* 644 */       if (this.index >= 0) this.index -= 1;
/* 645 */       return ShortLinkedOpenHashSet.this.key[this.curr];
/*     */     }
...
/*     */       }
/* 653 */       if (this.next == -1) {
/* 654 */         this.index = ShortLinkedOpenHashSet.this.size;
/* 655 */         return;
/*     */       }
/* 657 */       int pos = ShortLinkedOpenHashSet.this.first;
/* 658 */       this.index = 1;
/* 659 */       while (pos != this.prev) {
/* 660 */         pos = (int)ShortLinkedOpenHashSet.this.link[pos];
/* 661 */         this.index += 1;
/*     */       }
/*     */     }
/*     */ 
...
/*     */       {
/* 679 */         this.index -= 1;
/* 680 */         this.prev = ((int)(ShortLinkedOpenHashSet.this.link[this.curr] >>> 32));
/*     */       }
/*     */       else {
/* 683 */         this.next = ((int)ShortLinkedOpenHashSet.this.link[this.curr]);
/* 684 */       }ShortLinkedOpenHashSet.this.size -= 1;
/*     */ 
/* 687 */       if (this.prev == -1) ShortLinkedOpenHashSet.this.first = this.next;
/*     */       else
/* 689 */         ShortLinkedOpenHashSet.this.link[this.prev] ^= (ShortLinkedOpenHashSet.this.link[this.prev] ^ this.next & 0xFFFFFFFF) & 0xFFFFFFFF;
/* 690 */       if (this.next == -1) ShortLinkedOpenHashSet.this.last = this.prev;
/*     */       else
/* 692 */         ShortLinkedOpenHashSet.this.link[this.next] ^= (ShortLinkedOpenHashSet.this.link[this.next] ^ (this.prev & 0xFFFFFFFF) << 32) & 0x0; int pos = this.curr;
/*     */       int last;
/*     */       while (true) {
/* 696 */         pos = (last = pos) + 1 & ShortLinkedOpenHashSet.this.mask;
/* 697 */         while (ShortLinkedOpenHashSet.this.used[pos] != 0) {
/* 698 */           int slot = HashCommon.murmurHash3(ShortLinkedOpenHashSet.this.key[pos]) & ShortLinkedOpenHashSet.this.mask;
/* 699 */           if (last <= pos ? (last < slot) && (slot <= pos) : (last >= slot) && (slot > pos)) break;
/* 700 */           pos = pos + 1 & ShortLinkedOpenHashSet.this.mask;
/*     */         }
/* 702 */         if (ShortLinkedOpenHashSet.this.used[pos] == 0) break;
/* 703 */         ShortLinkedOpenHashSet.this.key[last] = ShortLinkedOpenHashSet.this.key[pos];
/* 704 */         if (this.next == pos) this.next = last;
/* 705 */         if (this.prev == pos) this.prev = last;
/* 706 */         ShortLinkedOpenHashSet.this.fixPointers(pos, last);
/*     */       }
/* 708 */       ShortLinkedOpenHashSet.this.used[last] = false;
/* 709 */       this.curr = -1;
/*     */     }
/*     */   }
/*     */ }
