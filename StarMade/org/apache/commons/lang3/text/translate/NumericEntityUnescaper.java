  ...
  extends CharSequenceTranslator
{
    ...
    throws IOException
  {
    ...
    if ((input.charAt(index) == '&') && (index < seqEnd - 2) && (input.charAt(index + 1) == '#'))
    {
      ...
      for (int end = start; (end < seqEnd) && (((input.charAt(end) >= '0') && (input.charAt(end) <= '9')) || ((input.charAt(end) >= 'a') && (input.charAt(end) <= 'f')) || ((input.charAt(end) >= 'A') && (input.charAt(end) <= 'F'))); end++) {}
      ...
      {
        char[] nfe = Character.toChars(entityValue);
        out.write(nfe[0]);
        out.write(nfe[1]);
      }
      else
