  ...
  extends CLObjectChild<CLDevice>
{
  ...
  public CLDevice getSubCLDevice(long local_id)
  {
    return (CLDevice)this.subCLDevices.getObject(local_id);
  }
  ...
  {
    for (int local_i = devices.position(); local_i < devices.limit(); local_i++)
    {
      long pointer = devices.get(local_i);
      if (pointer != 0L) {
