...
final class LinuxMouse
{
  ...
  LinuxMouse(long display, long window, long input_window)
  ...
  public void poll(boolean grab, IntBuffer coord_buffer, ByteBuffer buttons_buffer)
  {
    ...
    for (int local_i = 0; local_i < this.buttons.length; local_i++) {
      buttons_buffer.put(local_i, this.buttons[local_i]);
    }
  }
  
  private void putMouseEventWithCoords(byte button, byte state, int coord1, int coord2, int local_dz, long nanos)
  {
    this.event_buffer.clear();
    this.event_buffer.put(button).put(state).putInt(coord1).putInt(coord2).putInt(local_dz).putLong(nanos);
    this.event_buffer.flip();
    this.event_queue.putEvent(this.event_buffer);
    this.last_event_nanos = nanos;
  }
  
  private void setCursorPos(boolean grab, int local_x, int local_y, long nanos)
  {
  ...
  private int transformY(int local_y)
  {
    return nGetWindowHeight(this.display, this.window) - 1 - local_y;
  }
  ...
  {
    nWarpCursor(this.display, this.window, local_x, transformY(local_y));
  }
  
  private static native void nWarpCursor(long paramLong1, long paramLong2, int paramInt1, int paramInt2);
  
  private void handlePointerMotion(boolean grab, boolean warp_pointer, long millis, long root_window, int x_root, int y_root, int local_x, int local_y)
  {
    doHandlePointerMotion(grab, warp_pointer, root_window, x_root, y_root, local_x, local_y, millis * 1000000L);
  }
  ...
  private void putMouseEvent(boolean grab, byte button, byte state, int local_dz, long nanos)
  {
    if (grab) {
  ...
  private void resetCursor(int local_x, int local_y)
  {
    this.last_x = local_x;
    this.last_y = transformY(local_y);
  }
  
  private void handleWarpEvent(int local_x, int local_y)
  {
    resetCursor(local_x, local_y);
  }
