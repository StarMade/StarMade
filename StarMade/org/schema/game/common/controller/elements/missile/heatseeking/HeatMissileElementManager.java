...
public class HeatMissileElementManager
  extends MissileElementManager
{
  ...
  {
    super((short)46, (short)40, paramSegmentController);
  }
  ...
  {
    return new HeatMissileCollectionManager(paramclass_796, getSegmentController());
  }
  ...
  public void handle(class_755 paramclass_755)
  {
    ...
    for (int i = 0; i < getCollectionManagers().size(); i++)
    {
      ...
      int j = 0;
      if (((localHeatMissileCollectionManager = (HeatMissileCollectionManager)getCollectionManagers().get(i)).equalsControllerPos(this.controlledFrom)) && ((this.controlledFromOrig.equals(this.controlledFrom) | getControlElementMap().isControlling(this.controlledFromOrig, localHeatMissileCollectionManager.getControllerPos(), this.controllerId))))
      {
        for (j = 0; j < localHeatMissileCollectionManager.getCollection().size(); j++)
        {
          MissileUnit localMissileUnit;
          if ((localMissileUnit = (MissileUnit)localHeatMissileCollectionManager.getCollection().get(j)).canShoot())
          {
            ...
            {
              System.err.println("[SERVER] spawing heat missile " + localObject1);
              (localObject2 = new Transform()).setIdentity();
              ((Transform)localObject2).origin.set((Tuple3f)localObject1);
              getMissileController().addHeatMissile(getSegmentController(), (Transform)localObject2, new Vector3f(this.shootingDirTemp), localMissileUnit.getSpeed(), localMissileUnit.getBlastRadius(), localMissileUnit.getDamage(), localMissileUnit.getDistance());
            }
            (localObject2 = new Transform()).setIdentity();
            ((Transform)localObject2).origin.set((Tuple3f)localObject1);
            if (!getSegmentController().isOnServer())
        ...
        if ((localHeatMissileCollectionManager.getCollection().isEmpty()) && (clientIsOwnShip())) {
  ...
  public void updateToFullNT(NetworkObject paramNetworkObject)
  {
    if (getSegmentController().isOnServer()) {
      for (paramNetworkObject = 0; paramNetworkObject < getCollectionManagers().size(); paramNetworkObject++) {
        ((HeatMissileCollectionManager)getCollectionManagers().get(paramNetworkObject)).sendDistribution();
      }
    }
  }
  ...
  public void onActivate(class_796 paramclass_796, boolean paramBoolean)
  {
    ...
    for (int i = 0; i < getCollectionManagers().size(); i++)
    {
      Iterator localIterator = ((HeatMissileCollectionManager)getCollectionManagers().get(i)).getCollection().iterator();
